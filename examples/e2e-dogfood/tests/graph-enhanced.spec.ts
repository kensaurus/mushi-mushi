/**
 * Smoke test for enhanced Graph page.
 */
...
test.describe('Graph enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/graph?tab=overview`, { waitUntil: 'networkidle' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByText('GRAPH SNAPSHOT', { exact: true })).toBeVisible()
    await expect(page.getByLabel('Graph sections')).toBeVisible()
    await expect(page.getByLabel('Graph sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('explore tab loads (default)', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/graph`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Graph sections').getByRole('radio', { name: 'Explore' })).toBeChecked()
  })

  test('backend tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/graph?tab=backend`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Graph sections').getByRole('radio', { name: 'Backend' })).toBeChecked()
  })
})
