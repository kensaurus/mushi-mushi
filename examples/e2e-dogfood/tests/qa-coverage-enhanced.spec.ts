/**
 * Smoke test for enhanced QA Coverage page.
 */
...
test.describe('QA Coverage enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-qa-coverage')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('QA SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('QA coverage sections')).toBeVisible()
    await expect(page.getByLabel('QA coverage sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('stories tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/qa-coverage?tab=stories`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('QA coverage sections').getByRole('radio', { name: 'Stories' })).toBeChecked({ timeout: 15000 })
  })

  test('failing tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/qa-coverage?tab=failing`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('QA coverage sections').getByRole('radio', { name: 'Failing' })).toBeChecked({ timeout: 15000 })
  })
})
