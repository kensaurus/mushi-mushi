/**
 * Smoke test for enhanced Judge page.
 */
...
test.describe('Judge enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/judge`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-judge')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('JUDGE SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Judge sections')).toBeVisible()
    await expect(page.getByLabel('Judge sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
    await expect(page.getByRole('button', { name: 'Run judge now' }).first()).toBeVisible()
  })

  test('trend tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/judge?tab=trend`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Judge sections').getByRole('radio', { name: 'Trend' })).toBeChecked({ timeout: 15000 })
    await expect(page.getByText('Score trend (12w)', { exact: true })).toBeVisible({ timeout: 15000 })
  })

  test('evaluations tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/judge?tab=evaluations`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Judge sections').getByRole('radio', { name: 'Evaluations' })).toBeChecked({ timeout: 15000 })
    await expect(page.getByText('Recent evaluations', { exact: true })).toBeVisible({ timeout: 15000 })
  })
})
