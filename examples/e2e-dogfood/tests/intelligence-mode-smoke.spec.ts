/**
 * Smoke test: Intelligence mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:intelligence')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Intelligence mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-intelligence')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Intelligence sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Your weekly bug summary' })).toBeVisible()
  })

  test('Quick mode — reports/pipeline panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-intelligence')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Intelligence sections' })).toHaveCount(0)
