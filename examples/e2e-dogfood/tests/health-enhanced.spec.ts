/**
 * Smoke test for enhanced Health page.
 */
...
test.describe('Health enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/health`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-health')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('HEALTH SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Health sections')).toBeVisible()
    await expect(page.getByLabel('Health sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('llm tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/health?tab=llm`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Health sections').getByRole('radio', { name: 'LLM' })).toBeChecked({ timeout: 15000 })
    await expect(page.getByText('Per-function breakdown', { exact: true })).toBeVisible({ timeout: 15000 })
  })

  test('cron tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/health?tab=cron`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Health sections').getByRole('radio', { name: 'Cron' })).toBeChecked({ timeout: 15000 })
    await expect(page.getByText('Cron Jobs', { exact: true })).toBeVisible({ timeout: 15000 })
  })
})
