/**
 * Smoke test for enhanced Fixes page.
 */
...
test.describe('Fixes enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/fixes?tab=overview`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByText('FIXES SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Fixes sections')).toBeVisible()
    await expect(page.getByLabel('Fixes sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('pipeline tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/fixes?tab=pipeline`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Fixes sections').getByRole('radio', { name: 'Pipeline' })).toBeChecked({ timeout: 15000 })
  })

  test('attempts tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/fixes?tab=attempts`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Fixes sections').getByRole('radio', { name: 'Attempts' })).toBeChecked({ timeout: 15000 })
  })
})
