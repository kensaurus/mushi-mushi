/**
 * Smoke test for enhanced Storage page — tabs, banner, KPI strip.
 */
...
test.describe('Storage enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/storage`, { waitUntil: 'networkidle' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByRole('heading', { name: /Storage|BYO storage/i }).first()).toBeVisible()
    await expect(page.getByText('STORAGE SNAPSHOT')).toBeVisible()
    await expect(page.getByLabel('Storage sections')).toBeVisible()
    await expect(page.getByLabel('Storage sections').getByRole('radio', { name: 'Overview' })).toBeVisible()
  })

  test('configure tab shows provider select', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/storage?tab=configure`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Storage sections').getByRole('radio', { name: 'Configure' })).toBeChecked()
    await expect(page.getByLabel('Provider').first()).toBeVisible()
  })

  test('usage tab shows per-project table', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/storage?tab=usage`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Storage sections').getByRole('radio', { name: 'Usage' })).toBeChecked()
    await expect(page.getByText('Per-project usage')).toBeVisible()
  })
})
