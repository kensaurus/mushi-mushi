/**
 * Smoke test for enhanced Reports page.
 */
...
test.describe('Reports enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/reports?tab=overview`, { waitUntil: 'networkidle' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByRole('heading', { name: /bugs|Reports|Triage/i }).first()).toBeVisible()
    await expect(page.getByText('TRIAGE SNAPSHOT', { exact: true })).toBeVisible()
    await expect(page.getByLabel('Reports sections')).toBeVisible()
    await expect(page.getByLabel('Reports sections').getByRole('radio', { name: 'Overview' })).toBeVisible()
  })

  test('queue tab loads (default)', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/reports`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Reports sections').getByRole('radio', { name: 'Queue' })).toBeChecked()
  })

  test('severity tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/reports?tab=severity`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Reports sections').getByRole('radio', { name: 'Severity' })).toBeChecked()
  })
})
