/**
 * Smoke test for enhanced My feedback page.
 */
...
test.describe('Feedback enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/feedback`, { waitUntil: 'networkidle' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByRole('heading', { name: /My feedback/i }).first()).toBeVisible()
    await expect(page.getByText('FEEDBACK SNAPSHOT', { exact: true })).toBeVisible()
    await expect(page.getByLabel('Feedback sections')).toBeVisible()
    await expect(page.getByLabel('Feedback sections').getByRole('radio', { name: 'Overview' })).toBeVisible()
  })

  test('active tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/feedback?tab=active`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Feedback sections').getByRole('radio', { name: 'Active' })).toBeChecked()
  })

  test('shipped tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/feedback?tab=shipped`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Feedback sections').getByRole('radio', { name: 'Shipped' })).toBeChecked()
  })

  test('all tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/feedback?tab=all`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Feedback sections').getByRole('radio', { name: 'All' })).toBeChecked()
  })
})
