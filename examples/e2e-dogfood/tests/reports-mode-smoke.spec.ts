/**
 * Smoke test: Reports mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:reports')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Reports mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-reports')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Reports sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Bugs your users felt' })).toBeVisible()
  })

  test('Quick mode — queue only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-reports')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Reports sections' })).toHaveCount(0)
