/**
 * Smoke test for enhanced Inventory (User stories) page.
 */
...
test.describe('Inventory enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    ...
    await expect(page.getByText('INVENTORY SNAPSHOT', { exact: true })).toBeVisible()
    await expect(page.getByLabel('Inventory sections')).toBeVisible()
    await expect(page.getByLabel('Inventory sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('stories tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/inventory?tab=stories`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Inventory sections').getByRole('radio', { name: 'User stories' })).toBeChecked()
  })

  test('discovery tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/inventory?tab=discovery`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Inventory sections').getByRole('radio', { name: 'Discovery' })).toBeChecked()
  })
})
