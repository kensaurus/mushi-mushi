/**
 * Smoke test for enhanced Audit page — tabs, banner, KPI strip.
 */
...
test.describe('Audit enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    ...
    await expect(page.getByText('AUDIT SNAPSHOT')).toBeVisible()
    await expect(page.getByLabel('Audit sections')).toBeVisible()
    await expect(page.getByLabel('Audit sections').getByRole('radio', { name: 'Overview' })).toBeVisible()
  })

  test('log tab shows filter panel and table region', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/audit?tab=log`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Audit sections').getByRole('radio', { name: 'Log' })).toBeChecked()
    await expect(page.getByRole('combobox').first()).toBeVisible()
  })

  test('breakdown tab shows actor mix', async ({ page }) => {
    ...
    await expect(page.getByText('7-DAY ACTION MIX')).toBeVisible()
  })

  test('failure preset deep link opens log tab', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/audit?tab=log&action=fix.failed&since=24h`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Audit sections').getByRole('radio', { name: 'Log' })).toBeChecked()
    await expect(page.getByRole('combobox').first()).toHaveValue('fix.failed')
  })
})
