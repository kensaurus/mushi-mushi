/**
 * Smoke test: Repo mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:repo')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Repo mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await page.goto(`${ADMIN_URL}/repo`, { waitUntil: 'domcontentloaded' })
    await page.waitForTimeout(2500)

    await expect(page.getByTestId('mushi-page-repo')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Repo sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Your GitHub branches' })).toBeVisible()
  })

  test('Quick mode — branches panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-repo')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Repo sections' })).toHaveCount(0)
