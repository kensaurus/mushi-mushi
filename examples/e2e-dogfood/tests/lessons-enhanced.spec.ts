/**
 * Smoke test for enhanced Lessons page.
 */
...
test.describe('Lessons enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/lessons`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-lessons')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('LESSONS SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Lessons sections')).toBeVisible()
    await expect(page.getByLabel('Lessons sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('lessons tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/lessons?tab=lessons`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Lessons sections').getByRole('radio', { name: 'Lessons' })).toBeChecked({ timeout: 15000 })
  })

  test('clusters tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/lessons?tab=clusters`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Lessons sections').getByRole('radio', { name: 'Clusters' })).toBeChecked({ timeout: 15000 })
  })
})
