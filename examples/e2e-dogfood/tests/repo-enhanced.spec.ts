/**
 * Smoke test for enhanced Repo page.
 */
...
test.describe('Repo enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/repo?tab=overview`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByText('REPO SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Repo sections')).toBeVisible()
    await expect(page.getByLabel('Repo sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('branches tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/repo?tab=branches`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Repo sections').getByRole('radio', { name: 'Branches' })).toBeChecked({ timeout: 15000 })
  })

  test('activity tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/repo?tab=activity`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Repo sections').getByRole('radio', { name: 'Activity' })).toBeChecked({ timeout: 15000 })
  })
})
