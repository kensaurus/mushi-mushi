/**
 * Smoke test: Lessons mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:lessons')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Lessons mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-lessons')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Lessons sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Learn from past bugs' })).toBeVisible()
  })
  ...
  test('Quick mode — actionable panel only (no tabs, no snapshot)', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-lessons')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Lessons sections' })).toHaveCount(0)
