/**
 * Smoke test for enhanced Queue page.
 */
...
test.describe('Queue enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/queue?tab=overview`, { waitUntil: 'networkidle' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByText('QUEUE SNAPSHOT', { exact: true })).toBeVisible()
    await expect(page.getByLabel('Queue sections')).toBeVisible()
    await expect(page.getByLabel('Queue sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('backlog tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/queue?tab=backlog`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Queue sections').getByRole('radio', { name: 'Backlog' })).toBeChecked()
  })

  test('items tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/queue?tab=items`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Queue sections').getByRole('radio', { name: 'Items' })).toBeChecked()
  })
})
