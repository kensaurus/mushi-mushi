/**
 * Smoke test: Cost mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:cost')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Cost mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await page.goto(`${ADMIN_URL}/cost`, { waitUntil: 'domcontentloaded' })
    await page.waitForTimeout(2500)

    await expect(page.getByTestId('mushi-page-cost')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'LLM cost sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'What AI calls cost' })).toBeVisible()
  })
  ...
  test('Quick mode — actionable panel only (no tabs, no snapshot)', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-cost')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'LLM cost sections' })).toHaveCount(0)
