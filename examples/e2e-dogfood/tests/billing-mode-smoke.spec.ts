/**
 * Smoke test: Billing mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:billing')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Billing mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await page.goto(`${ADMIN_URL}/billing`, { waitUntil: 'domcontentloaded' })
    await page.waitForTimeout(2500)

    await expect(page.getByTestId('mushi-page-billing')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Billing sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Your plan & usage' })).toBeVisible()
  })

  test('Quick mode — usage panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-billing')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Billing sections' })).toHaveCount(0)
