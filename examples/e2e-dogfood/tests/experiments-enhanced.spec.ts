/**
 * Smoke test for enhanced Experiments page.
 */
...
test.describe('Experiments enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/experiments`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-experiments')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('EXPERIMENTS SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Experiments sections')).toBeVisible()
    await expect(page.getByLabel('Experiments sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('experiments tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/experiments?tab=experiments`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Experiments sections').getByRole('radio', { name: 'Experiments' })).toBeChecked({ timeout: 15000 })
  })

  test('new tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/experiments?tab=new`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Experiments sections').getByRole('radio', { name: 'New' })).toBeChecked({ timeout: 15000 })
