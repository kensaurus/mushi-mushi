/**
 * Smoke test: Explore mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:explore')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Explore mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-explore')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Explore sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Map your codebase' })).toBeVisible()
  })

  test('Quick mode — graph panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-explore')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Explore sections' })).toHaveCount(0)
