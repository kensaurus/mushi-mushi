/**
 * Smoke test for enhanced Iterate page.
 */
...
test.describe('Iterate enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/iterate`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-iterate')).toBeVisible({ timeout: 15000 })
    await expect(page.getByTestId('mushi-page-iterate').getByText('PDCA SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Iterate sections')).toBeVisible()
    await expect(page.getByLabel('Iterate sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('runs tab loads table section', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/iterate?tab=runs`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Iterate sections').getByRole('radio', { name: 'Runs' })).toBeChecked({
      timeout: 15000,
    })
    await expect(page.getByText('Run history', { exact: true })).toBeVisible({ timeout: 15000 })
  })

  test('new run tab loads form', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/iterate?tab=new`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Iterate sections').getByRole('radio', { name: 'New Run' })).toBeChecked({
      timeout: 15000,
    })
    await expect(page.getByText('Queue new run', { exact: true })).toBeVisible({ timeout: 15000 })
  })
})
