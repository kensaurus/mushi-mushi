/**
 * Smoke test for enhanced Drift page.
 */
...
test.describe('Drift enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/drift`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-drift')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('DRIFT SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Drift sections')).toBeVisible()
    await expect(page.getByLabel('Drift sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('findings tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/drift?tab=findings`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Drift sections').getByRole('radio', { name: 'Findings' })).toBeChecked({ timeout: 15000 })
  })

  test('scanner tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/drift?tab=scanner`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Drift sections').getByRole('radio', { name: 'Scanner' })).toBeChecked({ timeout: 15000 })
