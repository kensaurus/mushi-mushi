/**
 * Smoke test for enhanced Notifications page.
 */
...
test.describe('Notifications enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-notifications')).toBeVisible({ timeout: 15000 })
    await expect(
      page.getByTestId('mushi-page-notifications').getByText('NOTIFICATIONS SNAPSHOT', { exact: true }),
    ).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Notification sections')).toBeVisible()
    await expect(
      page.getByLabel('Notification sections').getByRole('radio', { name: 'Overview' }),
    ).toBeChecked()
  })

  test('inbox tab loads reporter inbox', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/notifications?tab=inbox`, { waitUntil: 'domcontentloaded' })
    await expect(
      page.getByLabel('Notification sections').getByRole('radio', { name: 'Inbox' }),
    ).toBeChecked({ timeout: 15000 })
    await expect(page.getByRole('heading', { name: 'Reporter inbox' })).toBeVisible({ timeout: 15000 })
  })

  test('setup tab loads pipeline checklist', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/notifications?tab=setup`, { waitUntil: 'domcontentloaded' })
    await expect(
      page.getByLabel('Notification sections').getByRole('radio', { name: 'Setup' }),
    ).toBeChecked({ timeout: 15000 })
    await expect(page.getByRole('heading', { name: 'Pipeline checklist' })).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('Reporter loop prerequisites')).toBeVisible()
  })
})
