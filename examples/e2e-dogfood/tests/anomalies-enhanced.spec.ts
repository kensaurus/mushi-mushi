/**
 * Smoke test for enhanced Anomalies page.
 */
...
test.describe('Anomalies enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/anomalies`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-anomalies')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('ANOMALIES SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Anomalies sections')).toBeVisible()
    await expect(page.getByLabel('Anomalies sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('anomalies tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/anomalies?tab=anomalies`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Anomalies sections').getByRole('radio', { name: 'Anomalies' })).toBeChecked({ timeout: 15000 })
  })

  test('detect tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/anomalies?tab=detect`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Anomalies sections').getByRole('radio', { name: 'Detect' })).toBeChecked({ timeout: 15000 })
    await expect(page.getByRole('heading', { name: 'Manual detection run' })).toBeVisible()
  })
})
