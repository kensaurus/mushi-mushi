/**
 * Smoke test for enhanced Onboarding (Get started) page.
 */
...
test.describe('Onboarding enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/onboarding`, { waitUntil: 'networkidle' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByRole('heading', { name: /Setup|Get started/i }).first()).toBeVisible()
    await expect(page.getByText('SETUP SNAPSHOT')).toBeVisible()
    await expect(page.getByLabel('Setup sections')).toBeVisible()
    await expect(page.getByLabel('Setup sections').getByRole('radio', { name: 'Overview' })).toBeVisible()
  })

  test('steps tab shows checklist region', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/onboarding?tab=steps`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Setup sections').getByRole('radio', { name: 'Steps' })).toBeChecked()
  })

  test('verify tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/onboarding?tab=verify`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Setup sections').getByRole('radio', { name: 'Verify' })).toBeChecked()
  })

  test('sdk tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/onboarding?tab=sdk`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Setup sections').getByRole('radio', { name: 'SDK' })).toBeChecked()
  })
})
