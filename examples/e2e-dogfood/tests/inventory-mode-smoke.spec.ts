/**
 * Smoke test: Inventory mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:inventory')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Inventory mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-inventory')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Inventory sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Map your app screens' })).toBeVisible()
  })
  ...
  test('Quick mode — stories panel only (no tabs, no snapshot)', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-inventory')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Inventory sections' })).toHaveCount(0)
