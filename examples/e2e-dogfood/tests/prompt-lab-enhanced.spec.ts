/**
 * Smoke test for enhanced Prompt Lab page.
 */
...
test.describe('Prompt Lab enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/prompt-lab?tab=overview`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByText('PROMPT LAB SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Prompt lab sections')).toBeVisible()
    await expect(page.getByLabel('Prompt lab sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('prompts tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/prompt-lab?tab=prompts`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Prompt lab sections').getByRole('radio', { name: 'Prompts' })).toBeChecked({ timeout: 15000 })
  })

  test('dataset tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/prompt-lab?tab=dataset`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Prompt lab sections').getByRole('radio', { name: 'Dataset' })).toBeChecked({ timeout: 15000 })
  })
})
