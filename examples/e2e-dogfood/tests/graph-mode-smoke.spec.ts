/**
 * Smoke test: Graph mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:graph')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Graph mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await page.goto(`${ADMIN_URL}/graph`, { waitUntil: 'domcontentloaded' })
    await page.waitForTimeout(2500)

    await expect(page.getByTestId('mushi-page-graph')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Graph sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'How bugs connect' })).toBeVisible()
  })

  test('Quick mode — map-only chrome (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-graph')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Graph sections' })).toHaveCount(0)
