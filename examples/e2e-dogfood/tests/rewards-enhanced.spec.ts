/**
 * Smoke test for enhanced Rewards page.
 */
...
const ORG_ID = '5de684b4-f64a-49e4-9973-4756ecbe5809'
...
test.describe('Rewards enhanced shell', () => {
  test.skip(
  ...
  test.beforeEach(async ({ page, request }) => {
    ...
    await page.addInitScript(
      ({ key, sessionPayload, projectId, orgId }) => {
        const expiresAt =
        ...
        window.localStorage.setItem('mushi:active_org_id', orgId)
      },
      {
        key: storageKey,
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/rewards`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-rewards')).toBeVisible({ timeout: 15000 })
    await expect(page.getByTestId('mushi-page-rewards').getByText('REWARDS SNAPSHOT', { exact: true })).toBeVisible({
      timeout: 15000,
    })
    await expect(page.getByLabel('Rewards sections')).toBeVisible()
    await expect(page.getByLabel('Rewards sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('rules tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/rewards?tab=rules`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Rewards sections').getByRole('radio', { name: 'Rules' })).toBeChecked({
      timeout: 15000,
    })
  })

  test('settings tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/rewards?tab=settings`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Rewards sections').getByRole('radio', { name: 'Settings' })).toBeChecked({
      timeout: 15000,
    })
  })
})
