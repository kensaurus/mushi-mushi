/**
 * Smoke test: Notifications mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:notifications')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Notifications mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-notifications')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Notification sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Updates for bug reporters' })).toBeVisible()
  })

  test('Quick mode — inbox panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-notifications')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Notification sections' })).toHaveCount(0)
