...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:mcp')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('MCP mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-mcp')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'MCP sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Connect your AI editor' })).toBeVisible()
  })

  test('Quick mode — setup/catalog panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-mcp')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'MCP sections' })).toHaveCount(0)
