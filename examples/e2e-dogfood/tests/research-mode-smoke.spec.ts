/**
 * Smoke test: Research mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:research')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Research mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-research')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Research sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Look up fixes online' })).toBeVisible()
  })
  ...
  test('Quick mode — actionable panel only (no tabs, no snapshot)', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-research')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Research sections' })).toHaveCount(0)
