/**
 * Smoke test: Experiments mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:experiments')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Experiments mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-experiments')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Experiments sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Run A/B tests' })).toBeVisible()
  })

  test('Quick mode — actionable panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-experiments')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Experiments sections' })).toHaveCount(0)
