/**
 * Smoke test for enhanced Anti-Gaming page.
 */
...
test.describe('Anti-gaming enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/anti-gaming?tab=overview`, { waitUntil: 'networkidle' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByText('ANTI-GAMING SNAPSHOT', { exact: true })).toBeVisible()
    await expect(page.getByLabel('Anti-gaming sections')).toBeVisible()
    await expect(page.getByLabel('Anti-gaming sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('devices tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/anti-gaming?tab=devices`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Anti-gaming sections').getByRole('radio', { name: 'Devices' })).toBeChecked()
  })

  test('events tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/anti-gaming?tab=events`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Anti-gaming sections').getByRole('radio', { name: 'Events' })).toBeChecked()
  })
})
