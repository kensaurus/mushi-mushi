/**
 * Smoke test for enhanced Query page — tabs, banner, KPI strip.
 */
...
test.describe('Query enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    ...
    await expect(page.getByText('QUERY SNAPSHOT')).toBeVisible()
    await expect(page.getByLabel('Query sections')).toBeVisible()
    await expect(page.getByLabel('Query sections').getByRole('radio', { name: 'Overview' })).toBeVisible()
  })

  test('ask tab shows composer', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/query?tab=ask`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Query sections').getByRole('radio', { name: 'Ask' })).toBeChecked()
    await expect(page.getByLabel('Question composer')).toBeVisible()
  })

  test('history tab shows library controls', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/query?tab=history`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Query sections').getByRole('radio', { name: 'History' })).toBeChecked()
    await expect(page.getByLabel('Switch between saved, recent, and team queries')).toBeVisible()
  })

  test('schema tab lists approved tables', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/query?tab=schema`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Query sections').getByRole('radio', { name: 'Schema' })).toBeChecked()
    await expect(page.getByText('Approved tables', { exact: true })).toBeVisible()
