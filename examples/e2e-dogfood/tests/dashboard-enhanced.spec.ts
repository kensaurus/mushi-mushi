/**
 * Smoke test for enhanced Dashboard page.
 */
...
test.describe('Dashboard enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /bug-fix loop|Dashboard|Bugs to fix/i }).first()).toBeVisible()
    await expect(page.getByText('LOOP SNAPSHOT', { exact: true })).toBeVisible()
    await expect(page.getByText('Backlog', { exact: true })).toBeVisible()
    await expect(page.getByLabel('Dashboard sections')).toBeVisible()
    await expect(page.getByLabel('Dashboard sections').getByRole('radio', { name: 'Overview' })).toBeVisible()
  })

  test('loop tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/dashboard?tab=loop`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Dashboard sections').getByRole('radio', { name: 'Loop' })).toBeChecked()
  })

  test('metrics tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/dashboard?tab=metrics`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Dashboard sections').getByRole('radio', { name: 'Metrics' })).toBeChecked()
  })

  test('health tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/dashboard?tab=health`, { waitUntil: 'networkidle' })
    await expect(page.getByLabel('Dashboard sections').getByRole('radio', { name: 'Health' })).toBeChecked()
  })
})
