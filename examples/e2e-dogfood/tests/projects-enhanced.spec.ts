/**
 * Smoke test for enhanced Projects page.
 */
...
test.describe('Projects enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-projects')).toBeVisible({ timeout: 15000 })
    await expect(
      page.getByTestId('mushi-page-projects').getByText('PROJECTS SNAPSHOT', { exact: true }),
    ).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Projects sections')).toBeVisible()
    await expect(
      page.getByLabel('Projects sections').getByRole('radio', { name: 'Overview' }),
    ).toBeChecked()
  })

  test('list tab loads project cards', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/projects?tab=list`, { waitUntil: 'domcontentloaded' })
    await expect(
      page.getByLabel('Projects sections').getByRole('radio', { name: 'Your projects' }),
    ).toBeChecked({ timeout: 15000 })
    await expect(page.getByRole('heading', { name: 'Your projects' })).toBeVisible({ timeout: 15000 })
  })

  test('create tab loads new project form', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/projects?tab=create`, { waitUntil: 'domcontentloaded' })
    await expect(
      page.getByLabel('Projects sections').getByRole('radio', { name: 'New project' }),
    ).toBeChecked({ timeout: 15000 })
    await expect(page.getByRole('heading', { name: 'Create a project' })).toBeVisible({ timeout: 15000 })
    await expect(page.getByPlaceholder('New project name (e.g. Acme iOS app)')).toBeVisible()
  })
})
