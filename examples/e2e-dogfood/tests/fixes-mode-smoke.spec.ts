/**
 * Smoke test: Fixes mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:fixes')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Fixes mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-fixes')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Fix sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Fixes ready to review' })).toBeVisible()
  })

  test('Quick mode — attempts panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-fixes')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Fix sections' })).toHaveCount(0)
