/**
 * Smoke test: Integrations mode UX + banner.
 */
...
test.describe('Integrations mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-integrations')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Integration sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Where fixed bugs go' })).toBeVisible()
  })

  test('Quick mode — platform cards only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-integrations')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Integration sections' })).toHaveCount(0)
