/**
 * Smoke test: Settings mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:settings')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Settings mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-settings')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Settings sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Tune your project' })).toBeVisible()
  })

  test('Quick mode — active panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-settings')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Settings sections' })).toHaveCount(0)
