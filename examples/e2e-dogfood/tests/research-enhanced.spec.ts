/**
 * Smoke test for enhanced Research page.
 */
...
test.describe('Research enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/research`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-research')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('RESEARCH SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Research sections')).toBeVisible()
    await expect(page.getByLabel('Research sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('search tab loads query form', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/research?tab=search`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Research sections').getByRole('radio', { name: 'Search' })).toBeChecked({ timeout: 15000 })
    await expect(page.getByLabel('Web query')).toBeVisible()
  })

  test('history tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/research?tab=history`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Research sections').getByRole('radio', { name: 'History' })).toBeChecked({ timeout: 15000 })
  })
})
