/**
 * Smoke test for enhanced Releases page.
 */
...
test.describe('Releases enhanced shell', () => {
  test.skip(
  ...
  test('overview loads banner, KPI strip, and tabs', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/releases`, { waitUntil: 'domcontentloaded' })
    await page.getByRole('button', { name: 'Dismiss' }).click({ timeout: 3000 }).catch(() => {})
    await expect(page.getByTestId('mushi-page-releases')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText('RELEASES SNAPSHOT', { exact: true })).toBeVisible({ timeout: 15000 })
    await expect(page.getByLabel('Releases sections')).toBeVisible()
    await expect(page.getByLabel('Releases sections').getByRole('radio', { name: 'Overview' })).toBeChecked()
  })

  test('published tab loads', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/releases?tab=published`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Releases sections').getByRole('radio', { name: 'Published' })).toBeChecked({ timeout: 15000 })
  })

  test('draft tab loads generate form', async ({ page }) => {
    await page.goto(`${ADMIN_URL}/releases?tab=draft`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByLabel('Releases sections').getByRole('radio', { name: 'Draft', exact: true })).toBeChecked({ timeout: 15000 })
    await expect(page.getByRole('heading', { name: 'Generate draft with AI' })).toBeVisible()
  })
})
