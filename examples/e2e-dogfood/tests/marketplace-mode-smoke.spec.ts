/**
 * Smoke test: Marketplace mode UX + banner.
 */
...
async function setupSession(page: import('@playwright/test').Page, request: import('@playwright/test').APIRequestContext) {
  ...
  await page.addInitScript(({ key, payload }) => {
    ...
    window.localStorage.removeItem('mushi:pagehelp:read:marketplace')
  }, { key: storageKey, payload: { ...session, expires_at: expiresAt } })
}

test.describe('Marketplace mode UX smoke', () => {
  test.skip(
  ...
  test('Beginner mode — tab strip + snapshot + plain title', async ({ page }) => {
    ...
    await expect(page.getByTestId('mushi-page-marketplace')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Marketplace sections' })).toBeVisible()
    await expect(page.getByText('At a glance', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Add webhook plugins' })).toBeVisible()
  })

  test('Quick mode — browse/installed panel only (no tabs, no snapshot)', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByTestId('mushi-page-marketplace')).toBeVisible()
    await expect(page.getByRole('radiogroup', { name: 'Marketplace sections' })).toHaveCount(0)
