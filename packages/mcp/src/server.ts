...
/**
 * The fix-context slice of a report detail row (GET /v1/admin/reports/:id).
 * The route composes `fix_packet` server-side, so get_fix_context and
 * triage_issue both read it from the report instead of a separate route.
 */
...
export function createMushiServer(config: MushiServerConfig): McpServer {
  ...
  const apiLog = createLogger({
    scope: 'mushi:mcp:api',
    level:
      (process.env.MUSHI_LOG_LEVEL as 'debug' | 'info' | 'warn' | 'error' | undefined) ?? 'info',
    destination: 'stderr',
  });
  ...
  async function apiCall<T = unknown>(path: string, options?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      const message =
        envelope?.error?.message ?? text.slice(0, 500) ?? `Request failed with ${res.status}`;
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      return wrappedJsonResult({ report: data }, 'report body');
    },
  );
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      return wrappedJsonResult({ report, ...fixContextOf(report) }, 'fix context');
    },
  );
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      const data = await apiCall<{ nodes: unknown[]; edges: unknown[] }>(
        `/v1/admin/graph/traverse?${params}`,
      );
    ...
    'get_graph_neighborhood',
    {
      ...
      inputSchema: z.object({
        seed: z
          .string()
          .describe(
            'Starting graph node id or label to expand around (e.g. an inventory Action id or component name)',
          ),
        depth: z
          .number()
          .int()
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      if (!pid)
        throw new MushiApiError(400, 'MISSING_PROJECT', 'projectId is required for get_inventory');
  ...
  server.registerTool(
    ...
    async (args) => {
      const pid = args.projectId ?? projectId;
      if (!pid)
        throw new MushiApiError(400, 'MISSING_PROJECT', 'projectId is required for diff_inventory');
    ...
    async (args) => {
      ...
      if (!pid)
        throw new MushiApiError(
          400,
          'MISSING_PROJECT',
          'projectId is required for list_gate_findings',
        );
      const q = new URLSearchParams();
      if (args.gate) q.set('gate', args.gate);
      if (args.severity) q.set('severity', args.severity);
      const suffix = q.toString() ? `?${q}` : '';
      const data = await apiCall<{ runs: unknown[]; findings: unknown[] }>(
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      return jsonText({
        project_id: pid,
        preflight:
          preflightRes.status === 'fulfilled'
            ? preflightRes.value
            : { error: String(preflightRes.reason) },
        activation:
          activationRes.status === 'fulfilled'
            ? activationRes.value
            : { error: String(activationRes.reason) },
      });
    },
  );
    ...
    'get_pipeline_logs',
    {
      ...
      inputSchema: z.object({
        ...
        since: z
          .string()
          .optional()
          .describe('ISO-8601 timestamp — return only entries after this time.'),
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      return jsonText({
        evidence,
        reporter_thread: timeline ?? {
          error: String((timelineRes as PromiseRejectedResult).reason),
        },
      });
    },
  );
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      const reportPath = `/v1/admin/reports/${encodeURIComponent(args.report_id)}`;
      ...
      const similarBugs =
        similarRes.status === 'fulfilled' && similarRes.value
          ? (similarRes.value.results ?? []).filter((r) => r.reportId !== args.report_id).slice(0, 5)
          : null;
    ...
    'dispatch_fix',
    {
      ...
      inputSchema: z.object({
        ...
        idempotencyKey: z
          ...
          .optional()
          .describe(
            'Optional RFC 4122 UUID. Resend the same key to safely retry without dispatching a duplicate fix job (Idempotency-Key IETF draft).',
          ),
        inventoryActionNodeId: z
          .string()
          .uuid()
          .optional()
          .describe(
    ...
    'trigger_judge',
    {
      ...
      inputSchema: z.object({
        limit: z
          .number()
          .optional()
          .describe('Max reports to judge in this batch (default 25, max 100)'),
        projectId: z
          .string()
          .optional()
    ...
    async (args) => {
      ...
      if (!pid)
        throw new MushiApiError(
          400,
          'MISSING_PROJECT',
          'projectId is required for test_gen_from_report',
        );
      const data = await apiCall(
        `/v1/admin/inventory/${pid}/test-gen/from-report/${args.reportId}`,
        {
          method: 'POST',
          body: JSON.stringify({}),
        },
    ...
    'refresh_ci',
    {
      ...
      outputSchema: z.object({
        ...
        check_run_conclusion: z
          .string()
          .nullable()
          .describe('success | failure | neutral | null while pending'),
        check_run_updated_at: z
          .string()
          .nullable()
      ...
      contents: [
        {
          uri: 'project://stats',
          mimeType: 'application/json',
          text: JSON.stringify(await apiCall('/v1/admin/stats'), null, 2),
        },
      ],
    }),
  );
      ...
      contents: [
        {
          ...
          mimeType: 'application/json',
          text: JSON.stringify(await apiCall('/v1/admin/settings'), null, 2),
        },
      ],
    }),
  );
  ...
  server.registerResource(
    ...
    {
      description:
        'PDCA health snapshot — stage counts, bottleneck, recent activity (same payload the admin console polls)',
    },
    async () => ({
      contents: [
        {
          uri: 'project://dashboard',
          mimeType: 'application/json',
          text: JSON.stringify(await apiCall('/v1/admin/dashboard'), null, 2),
        },
      ],
    }),
  );
      ...
      contents: [
        {
          ...
          text: projectId
            ? JSON.stringify(
                await apiCall(
                  `/v1/admin/projects/${encodeURIComponent(projectId)}/evolution-history`,
                ),
                null,
                2,
              )
    ...
    'map_user_stories',
    {
      ...
      inputSchema: z.object({
        ...
        maxPages: z
          ...
          .max(50)
          .optional()
          .describe('Max pages to crawl (default 20)'),
        provider: z
          .enum(['firecrawl', 'browserbase'])
          .optional()
          .describe('Crawl provider (default: firecrawl)'),
        cursorCloudRefine: z
          .boolean()
          .optional()
      ...
      const data = await apiCall<{ runId: string; status: string }>(
        `/v1/admin/inventory/${projectId}/map-from-live`,
        {
          ...
          body: JSON.stringify({
            ...
            max_pages: maxPages,
            provider,
            cursor_cloud_refine: cursorCloudRefine,
          }),
        },
      );
    ...
    'generate_tdd_from_story',
    {
      ...
      inputSchema: z.object({
        ...
        storyNodeId: z
          .string()
          .describe(
            'User story node id/slug from accepted inventory (from map_user_stories or inventory.yaml)',
          ),
        automationMode: z
          .enum(['auto', 'review', 'approve'])
          .optional()
          .describe('Approval gate for the generated qa_story (default review → pending_review)'),
        baseUrl: z
          .string()
          .url()
          .optional()
          .describe('Override the live app URL the Playwright test should target'),
        openPr: z
          .boolean()
          .optional()
      ...
      outputSchema: z.object({
        ...
        prUrl: z
          .string()
          .nullable()
          .describe('GitHub PR URL when openPr=true and GitHub is connected'),
        approvalStatus: z
          .string()
          .describe('pending_review | approved — follows automationMode and project defaults'),
        needsHumanReview: z
          .boolean()
  ...
  server.registerTool(
    ...
    async ({ projectId }) => {
      const data = await apiCall<unknown>(
        `/v1/admin/byok/keys?project_id=${encodeURIComponent(projectId)}`,
      );
      return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
    },
  );
    ...
    'add_byok_key',
    {
      ...
      inputSchema: z.object({
        ...
        priority: z
          ...
          .max(10_000)
          .optional()
          .describe('Priority for ordering (lower = higher priority)'),
        baseUrl: z
          .string()
          .url()
          .max(2048)
  ...
  server.registerTool(
    ...
    async ({ project_id, include_logs = true }) => {
      ...
      const result = {
        schema:
          schemaRes.status === 'fulfilled' ? schemaRes.value : { error: String(schemaRes.reason) },
        advisors:
          advisorsRes.status === 'fulfilled'
            ? advisorsRes.value
            : { error: String(advisorsRes.reason) },
        logs:
    ...
    'analyze_codebase_impact',
    {
      ...
      inputSchema: z.object({
        ...
        paths: z
          .array(z.string())
          .optional()
          .describe('Changed repo-relative file paths to analyze'),
        source: z
          .enum(['last_push', 'compare', 'fix'])
          .optional()
          .describe('Auto-resolve changed paths instead of manual paths'),
        compare: z
          .string()
          .optional()
      ...
      outputSchema: z.object({
        ...
        source: z
          .string()
          .describe('How changed_paths were resolved (manual, last_push, compare, fix, …)'),
        affected_file_paths: z
          .array(z.string())
          .describe('Indexed files that import or depend on changed paths'),
        affected_node_ids: z
          .array(z.string())
          .describe('Codebase graph node ids impacted by the change'),
        meta: z
          .unknown()
          .nullable()
          .optional()
    ...
    async (args) => {
      ...
      if (!qs.toString()) {
        throw new MushiApiError(
          400,
          'BAD_REQUEST',
          'Provide paths, source=last_push, compare, or fix_id',
        );
      }
      const data = await apiCall<{
  ...
  if (grantedScopes !== undefined || featureFilter !== 'all') {
    ...
    const allSpecs = [...TOOL_CATALOG, ...TDD_TOOL_CATALOG, ...CODEBASE_TOOL_CATALOG];
    for (const spec of allSpecs) {
      if (
        grantedScopes !== undefined &&
        !(grantedScopes as readonly string[]).includes(spec.scope)
      ) {
