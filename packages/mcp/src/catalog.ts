...
export const TOOL_CATALOG: ToolSpec[] = [
  ...
  },
  {
    ...
    description:
      'Return the full evidence package for a single bug report covering all three observability pillars: ' +
      '(1) LOGS — console_logs (error/warn/info/debug entries with timestamps), ' +
      'breadcrumbs (SDK ring buffer: navigation, clicks, network, lifecycle events with category/level), ' +
      'repro_timeline (merged SDK event stream: route/click/request/log/screen), ' +
      'and the reporter\'s own comments thread; ' +
      '(2) TRACES — network_requests (SDK-captured fetch/XHR with method/status/duration/traceId), ' +
      'backend_spans (server-side spans joined by W3C trace_id: name/duration_ms/parentSpanId/status), ' +
      'and Sentry trace correlation IDs (sentry_trace_id, sentry_event_id) for deeplinks; ' +
      '(3) METRICS — performance_metrics (Web Vitals snapshot: LCP/CLS/INP/TTFB/FCP + INP attribution + ' +
...
export const RESOURCE_CATALOG: ResourceSpec[] = [
  {
    ...
    description:
      'Loop health snapshot — stage counts, bottleneck, recent activity (same payload the admin console polls).',
    scope: 'mcp:read',
  },
  {
    name: 'project_stats',
...
export const PROMPT_CATALOG: PromptSpec[] = [
  {
    ...
    description:
      'Turn a Mushi report into a one-line root cause, smallest file set, repro steps, and blast-radius warnings.',
...
export const TDD_TOOL_CATALOG: ToolSpec[] = [
  ...
  },
  {
    ...
    scope: 'mcp:write',
    hints: { readOnly: false, destructive: false, idempotent: true, openWorld: true },
    useCase: 'Re-test a quarantined OpenAI key after fixing its account quota.',
  },
  {
    ...
    scope: 'mcp:write',
    hints: { readOnly: false, destructive: true, idempotent: true, openWorld: true },
    useCase: 'Remove a rotated or compromised provider key from the runtime pool.',
  },
  {
