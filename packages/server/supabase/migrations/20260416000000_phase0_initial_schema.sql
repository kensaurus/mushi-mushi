...
create index if not exists idx_api_keys_hash on project_api_keys(key_hash) where active = true;
...
create table if not exists reports (
  ...
  status text not null default 'pending' check (
    status in ('pending', 'submitted', 'classified', 'grouped', 'fixing', 'fixed', 'dismissed')
  ),
...
create index if not exists idx_reports_reporter on reports(reporter_token);
