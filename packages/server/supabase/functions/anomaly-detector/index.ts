 ...
 * Cron: pg_cron (mushi-anomaly-detector, hourly) posts { trigger: 'cron' } with no
 * project_id; that walks every project. Until 2026-09-22 the cron call was
 * refused (401 at the gateway, then 400 for the missing project_id), so the
 * hourly scan had never run.
 */
...
/** Detect anomalies in one project's recent metric series; returns inserted detection ids. */
...
Deno.serve(
  withSentry(async (req: Request) => {
    ...
    if (projectId) {
      const ids = await analyzeProject(db, projectId, metricFilter, lookbackHours)
      return json({ ok: true, anomalies: ids.length, ids })
    }

    if (body.trigger !== 'cron') return json({ error: 'project_id required' }, 400)

    // Hourly scan: every project; one without metric data in the window
    // returns after a single indexed query.
    const { data: projects, error } = await db.from('projects').select('id')
    if (error) return json({ ok: false, error: error.message }, 500)
    let anomalies = 0
    for (const p of projects ?? []) {
      anomalies += (await analyzeProject(db, p.id as string, metricFilter, lookbackHours)).length
    }
    return json({ ok: true, projects: projects?.length ?? 0, anomalies })
  }),
)
