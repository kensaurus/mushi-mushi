...
const AskMushiReplySchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    ...
    options: z.array(z.string().min(1).max(80)).min(2).max(4).describe('2–4 chip-shaped option labels the user can click.'),
  }),
])
...
async function resolveMentions(
  ...
  for (const [, kind, id] of uniq) {
    try {
      ...
      log.warn('ask-mushi mention resolution failed', { kind, id, error: err instanceof Error ? err.message : String(err) })
    }
  }
  return out
}
...
function buildAskMushiSystemPrompt({
  ...
  const intentLine = intent === 'default'
    ? ''
  ...
  const mentionsBlock = resolvedMentions.length > 0
...
app.post('/v1/admin/ask-mushi/messages', jwtAuth, async (c) => {
  ...
  const db = getServiceClient()
  const { activeProject, userProjectIds, recentReportsBlock } =
    await loadAskMushiContextData(db, userId)
  ...
  const userInsert = lastUser
    ? db.from('ask_mushi_messages').insert({
        thread_id: threadId,
        user_id: userId,
  ...
  const maxTokens = intent === 'tldr' ? 250
    : intent === 'long' ? 1200
    : intent === 'pr-summary' ? 1200
    : 600
  ...
  try {
    ...
    if (!reply) {
      return c.json({ ok: false, error: { code: 'LLM_ERROR', message: 'Empty model reply' } }, 500)
    }
    ...
    void db.from('ask_mushi_messages').insert({
      thread_id: threadId,
      user_id: userId,
      project_id: activeProject?.id ?? null,
      route,
  ...
  return streamSSE(c, async (stream) => {
    ...
    try {
      ...
      void db.from('ask_mushi_messages').insert({
        ...
        if (error) log.warn('ask_mushi stream assistant insert failed', { error: error.message })
      })
...
app.get('/v1/admin/ask-mushi/threads', jwtAuth, async (c) => {
  ...
  let q = db
    ...
    .select('thread_id, role, content, route, page_title, project_id, created_at, model, cost_usd')
    .eq('user_id', userId)
    .order('created_at', { ascending: false })
    .limit(limit * 6)
...
app.get('/v1/admin/ask-mushi/threads/:id', jwtAuth, async (c) => {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(id)) {
    return c.json({ ok: false, error: { code: 'BAD_REQUEST', message: 'invalid thread id' } }, 400)
  }
  const db = getServiceClient()
  const { data, error } = await db
    .from('ask_mushi_messages')
    .select('id, thread_id, role, content, route, page_title, project_id, model, fallback_used, input_tokens, output_tokens, cache_read_tokens, cache_create_tokens, cost_usd, latency_ms, langfuse_trace_id, meta, created_at')
    .eq('user_id', userId)
    .eq('thread_id', id)
    .order('created_at', { ascending: true })
  if (error) {
    if (error.code === '42P01') return c.json({ ok: true, data: { messages: [], degraded: 'schema_pending' } })
    return dbError(c, error)
  }
...
app.get('/v1/admin/ask-mushi/mentions', jwtAuth, async (c) => {
  ...
  try {
    ...
    log.warn('ask-mushi mentions: reports search failed', { error: err instanceof Error ? err.message : String(err) })
  }
  ...
  try {
    ...
    log.warn('ask-mushi mentions: fixes search failed', { error: err instanceof Error ? err.message : String(err) })
  }
