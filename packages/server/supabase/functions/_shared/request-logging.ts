 ...
 * - Keeps 5xx access logs out of Sentry; route exceptions report separately
 *
 * USAGE:
...
export function requestLoggingMiddleware() {
  return async (c: Context, next: Next) => {
    ...
    const doneMeta = {
      ...
      ...(logContext?.userId && logContext.authMethod === 'jwt'
    ...
    if (status >= 500) {
      reqLog.error('request.done', { ...doneMeta, sentry: false });
    } else if (status >= 400) {
