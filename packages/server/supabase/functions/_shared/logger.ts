 ...
 * - Errors/fatals forward to Sentry via reportMessage
 *
 * USAGE:
...
function detectLevel(): LogLevel {
  const raw = (readEnv('MUSHI_LOG_LEVEL') ?? readEnv('LOG_LEVEL') ?? 'info')
    .trim()
    .toLowerCase()
...
function redactString(value: string): string {
  ...
  if (value.length > 8 && /^[A-Za-z0-9+/=_-]{24,}$/.test(value)) return `[redacted:${value.slice(-4)}]`
  return value
}
