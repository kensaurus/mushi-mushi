 ...
 *   accidentally drops `byok` from Indie or grants `sso` on Free Cloud. CI
...
// ─── Catalog mirror ────────────────────────────────────────────────────────
// Mirrors the active diagnosis-metered catalog repaired by
// `20260806070334_restore_active_plan_entitlements.sql`. Kept in sync
// MANUALLY: if the migration changes, this constant has to change too.
// Do NOT loosen the assertions without also auditing the migration.
...
const PRO: PricingPlan = {
  ...
  included_reports_per_month: null,
  overage_unit_amount_decimal: null,
  included_diagnoses_per_month: 2_000,
...
const ALL_PLANS = [FREE_CLOUD, INDIE, PRO, ENTERPRISE];
...
const planState = {
  active: FREE_CLOUD as PricingPlan,
};
...
vi.mock('../../supabase/functions/_shared/plans.ts', async () => {
  return {
    ...
    getPlan: async (id: string) => ALL_PLANS.find((p) => p.id === id) ?? FREE_CLOUD,
    resolvePlanFromSubscription: async () => planState.active,
  };
});

const { requireFeature, minimumPlanFor, GATED_ROUTES } =
  await import('../../supabase/functions/_shared/entitlements.ts');

// ─── Helper: fake Hono context ─────────────────────────────────────────────
function makeCtx(
  overrides: { userId?: string; projectId?: string; path?: string; method?: string } = {},
) {
...
describe('minimumPlanFor', () => {
  it('returns the cheapest plan that grants each gated flag', async () => {
    expect((await minimumPlanFor('byok'))?.id).toBe('indie');
    expect((await minimumPlanFor('plugins'))?.id).toBe('indie');
    expect((await minimumPlanFor('audit_log'))?.id).toBe('indie');
    expect((await minimumPlanFor('sso'))?.id).toBe('enterprise');
...
describe('requireFeature middleware (gated-route × plan-tier matrix)', () => {
  ...
  for (const row of matrix) {
    ...
    >) {
      it(`${row.plan.id} → ${flag}: ${expectedAllowed ? 'allowed (200)' : 'blocked (402)'}`, async () => {
        ...
        await requireFeature(flag)(
          ctx as unknown as Parameters<ReturnType<typeof requireFeature>>[0],
          next,
        );
        ...
        if (expectedAllowed) {
          ...
          // Free Cloud/Indie blocked on a flag → upgrade_to MUST be set
