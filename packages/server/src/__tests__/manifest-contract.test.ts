 ...
 *
 * The match is purely structural (no Deno runtime, no live HTTP) so the test
 * runs in vanilla Vitest without needing the edge function to be served.
...
describe('auth manifest contract', () => {
  ...
  it('every advertised endpoint exists as a registered route', () => {
    ...
    expect(missing, `Manifest advertises endpoints with no Hono route: ${missing.join(', ')}`).toEqual([])
  })
  ...
  it('extracts the two endpoints implemented', () => {
    expect(advertised).toEqual(
      expect.arrayContaining([
        '/v1/admin/auth/token',
        '/v1/admin/projects/:id/keys/rotate',
      ]),
    )
  })
})
