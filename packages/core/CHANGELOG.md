...
- b858f5d: Internal: dedupe the ingest endpoint URL — `region.ts` now imports the canonical `DEFAULT_API_ENDPOINT` from `api-client.ts` instead of hardcoding the Supabase host in three places. No behavior change (all regions currently resolve to the same US gateway); removes a self-hoster drift risk.
...
- b0820ca: Harden `getSubtle()` to require a full SubtleCrypto surface (`digest`, `importKey`, `sign`) before using the Web Crypto fast path. Fixes Hermes partial `crypto.subtle` polyfills that threw `subtle.digest is not a function` (YEN-YEN-MOBILE-3T).
...
- 90bc9d5: Add widget-theme, digest, and project-scoped reporter-token exports consumed by the web + React Native banner SDKs:
...
- c4798cb: **Fix: SHA-256/HMAC-SHA-256 no longer crashes on Hermes (React Native)**
  ...
  This fixes `YEN-YEN-MOBILE-3R`: `ReferenceError: Property 'crypto' doesn't exist`
  that crashed `listMyReports()` on Android Hermes (`react-native@0.83.6`,
