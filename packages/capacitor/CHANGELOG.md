...
- 90bc9d5: Use the project-scoped reporter token (`getReporterToken(projectId)`) for submit, My Reports, comments, replies, and reopen so multi-project Capacitor hosts on the same origin no longer share a single anonymous reporter identity. Adds an explicit config guard on the reporter-inbox methods.
...
- de46c73: Fix `BreadcrumbCollector` iOS: clamp `maxMessageLength` floor to 1 instead of
