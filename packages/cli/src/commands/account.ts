...
/**
 * Runs the browser-guided device-auth login flow (RFC 8628), or the
 * non-interactive `--api-key` path. Extracted from the `login` command's
 * action so other commands (e.g. `setup`) can trigger it inline when no
 * credentials are configured yet, instead of erroring out and telling the
 * user to run `mushi login` first.
 */
export async function runLogin(opts: { apiKey?: string; endpoint?: string; projectId?: string; browser?: boolean; upgradeScope?: boolean }): Promise<void> {
