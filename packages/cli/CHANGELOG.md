...
- 0c1639b: One-command self-hosting, verified snippet insertion, and a doctor that tells you how to fix what it finds.
...
- 3fb91ad: Harden `mushi init` env-file writes against a check-then-read (TOCTOU) race — the file is now read directly with a fallback to empty on miss, instead of `existsSync()`-then-`readFileSync()`.
...
- 7533082: Fix the unscoped `mushi-mushi` launcher: any non-`init` subcommand (`setup`,
