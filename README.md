...
[Quick start](#quick-start) · [Live admin demo](https://kensaur.us/mushi-mushi/admin/) · [Docs](https://kensaur.us/mushi-mushi/docs/) · [Marketing landing](https://kensaur.us/mushi-mushi/) · [Self-hosting](./SELF_HOSTED.md) · [Architecture](#architecture)
...
<tr>
  <td width="50%" valign="top">
    <a href="https://kensaur.us/mushi-mushi/admin/"><img src="./docs/screenshots/quickstart-dark.png" alt="Quickstart mode dashboard — three-page sidebar (Setup, Bugs to fix, Fixes ready) with verb-led labels and zero PDCA jargon, 'You're caught up · No new bugs waiting' top banner with 'Open inbox →' primary action, 'Bugs to fix' page heading, Next-Best-Action strip '2 failed fixes need retry · Open Fixes →' (only the two items the user actually needs to act on), and the same live PDCA canvas below (Do node marked CURRENT FOCUS, '2 fixes in flight', '2 failed fixes need retry')" /></a>
  ...
  <td width="50%" valign="top">
    <a href="https://kensaur.us/mushi-mushi/admin/"><img src="./docs/screenshots/first-run-tour-dark.png" alt="First-run interactive tour — spotlight cutout around the Plan tile, dark backdrop dimming everything else, coach-mark panel reading 'TOUR - 1 OF 5 · Plan — bugs your users felt · Each tile is a stage of the loop. Plan is where real user complaints land, get classified, and get scored by severity. Click any tile to drill in.' with a Next button and a Don't show again link." /></a>
...
<tr>
  <td width="50%" valign="top">
    <a href="https://kensaur.us/mushi-mushi/admin/onboarding"><img src="./docs/screenshots/onboarding-dark.png" alt="Plug-n-play onboarding wizard with PDCA storyboard — four cards across the top (Plan capture, Do dispatch, Check verify, Act standardise) showing where the loop starts and ends, then a 'You're set up · 4/4 required · 100%' progress bar above a checklist of Done steps (project, API key, SDK install, first bug report, GitHub) and an Add API key CTA on the optional Anthropic step." /></a>
...
<tr>
  <td width="50%" valign="top">
    <a href="https://kensaur.us/mushi-mushi/admin/anti-gaming"><img src="./docs/screenshots/anti-gaming-dark.png" alt="Anti-gaming dashboard — Decide/Act/Verify hero (Decide tile 'Flagged devices need review · 4 flagged · 0 cross-account', Act tile 'All clear', Verify tile 'Latest enforcement activity · 53 recent events · 4 collapsed'), KPI strip (Tracked devices 4 -flat vs 7d avg, Flagged 4 ↑30 vs 7d avg, Cross-account 0 ↑30 vs 7d avg, Total reports 65), Flagged devices table with rows per fingerprint (fp:1f0b09790709b8a0..., 14 tokens · 1 IP · 14 reports, 'Multi-account: 4 reporter tokens from same device', First seen 4/23/2026), each with Details / Unflag actions, search input top-right" /></a>
  ...
  <td width="50%" valign="top">
    <a href="https://kensaur.us/mushi-mushi/admin/queue"><img src="./docs/screenshots/queue-dark.png" alt="Processing queue (DLQ) — Decide/Act/Verify hero ('Queue is healthy · 0 running · 0 pending · No stuck work, no dead letters', 'All clear', 'Latest throughput snapshot 2026-04-23 · 20 completed · 0 failed'), top-right batch ops (completed dropdown, All Stage dropdown, Retry page (25), Flush queued, Recover stranded), KPI cards (Pending 0, Running 0, Completed 67 ↑2853%, Failed 0, Dead letter 0), 14d daily throughput histogram (created/completed/failed colour-coded), per-stage backlog bar (stage1 67), and per-job rows showing 'stage1 - Completed · 6/3 attempts · user-story:1776924091355 — the login button does nothing and the page flickers · Created 7 hours ago - last attempt 7 hours ago' with a Retry button" /></a>
...
<tr>
  <td width="50%" valign="top">
    <a href="https://kensaur.us/mushi-mushi/admin/sso"><img src="./docs/screenshots/sso-dark.png" alt="SSO Configuration page — heading 'SSO Configuration · Configure SAML or OIDC for your team. JIT-provisioning on first login is enabled by default.', About SSO collapsed accordion, ADD IDENTITY PROVIDER card with SAML 2.0 dropdown, Provider name input (e.g. Okta), Metadata URL (required), Entity ID (optional, parsed from metadata), Email domains (comma-separated, e.g. acme.com,acme.io), 'Add Provider' yellow button, helper text 'On submit, Mushi calls the Supabase Auth Admin API to register the SAML provider. We surface the resulting ACS URL + Entity ID below for you to paste into your IdP.', empty state 'No identity providers configured' below" /></a>
...
**Published:** active SDK releases now span core / web / React / Vue / Svelte / Angular / React Native at `0.6.x`, the launcher at `0.6.x`, CLI / create packages at `0.5.x`, Capacitor at `0.4.x`, Node / MCP at `0.3.x`, native iOS / Android / Flutter at `0.3.x`, and plugins / adapters / MCP-CI / wasm at `0.2.x`. The hosted admin console lives at [`kensaur.us/mushi-mushi/admin/`](https://kensaur.us/mushi-mushi/admin/), the marketing landing at [`kensaur.us/mushi-mushi/`](https://kensaur.us/mushi-mushi/), and the docs at [`kensaur.us/mushi-mushi/docs/`](https://kensaur.us/mushi-mushi/docs/) — all three auto-deploy on every push to `master` (admin/docs to S3 + CloudFront, cloud to Vercel via CloudFront proxy).
...
1. Sign up at **[kensaur.us/mushi-mushi](https://kensaur.us/mushi-mushi/)** (the marketing landing) and click **Start free** to create your account
2. Open the admin console at **[kensaur.us/mushi-mushi/admin](https://kensaur.us/mushi-mushi/admin/)** → create a project → copy your `projectId` and `apiKey`
