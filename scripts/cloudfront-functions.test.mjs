...
/**
 * Load a CloudFront function's `handler` from disk — via the SAME
 * comment-stripping build (build-cf-function.mjs) that deploy workflows
 * publish, so every behavior test below exercises the deployable artifact,
 * not just the commented source.
 */
function loadHandler(filename) {
  const src = stripSource(readFileSync(join(__dirname, filename), 'utf8'));
  // CloudFront functions are plain scripts with a top-level `handler`.
  // eslint-disable-next-line no-new-func
  return new Function('event', `${src}\nreturn handler(event);`);
}

describe('CloudFront Function size gate', () => {
  ...
  for (const f of files) {
    it(`${f} builds under ${DEFAULT_MAX_BYTES} bytes`, () => {
      const built = stripSource(readFileSync(join(__dirname, f), 'utf8'));
      const size = Buffer.byteLength(built);
      assert.ok(
        size < DEFAULT_MAX_BYTES,
        `${f}: built artifact is ${size} bytes (gate ${DEFAULT_MAX_BYTES}, hard CloudFront limit ${CF_HARD_LIMIT_BYTES})`,
      );
    });
  }
});
...
describe('cloudfront-mushi-apex-redirect', () => {
  ...
  it('redirects /reports/uuid to admin (regression)', () => {
    ...
    assert.equal(
      ...
      '/mushi-mushi/admin/reports/a8054224-5d19-45e3-8c2b-1ad79182f761',
    );
  });
...
describe('cloudfront-mushi-hosted-mcp', () => {
  ...
  it('forwards AS metadata GET to Supabase origin', () => {
    const out = router(
      req('/mushi-mushi/hosted-mcp/.well-known/oauth-authorization-server', '', 'GET'),
    );
  ...
  it('forwards AS metadata HEAD to Supabase (Smithery RFC 8414)', () => {
    const out = router(
      req('/mushi-mushi/hosted-mcp/.well-known/oauth-authorization-server', '', 'HEAD'),
    );
    assert.equal(out.uri, '/.well-known/oauth-authorization-server');
    assert.equal(out.statusCode, undefined);
  });
  ...
  it('rewrites POST to Supabase path prefix', () => {
    const out = router(
      req('/mushi-mushi/hosted-mcp', '', 'POST', 'application/json, text/event-stream'),
    );
    assert.equal(out.uri, '/');
    assert.equal(out.statusCode, undefined);
  });
  ...
  it('302 OAuth authorize to Smithery callback at edge', () => {
    ...
    assert.match(
      out.headers.location.value,
      /^https:\/\/smithery\.run\/oauth\/callback\?code=mushi-scan-/,
    );
  ...
  it('serves origin PRM at well-known path', () => {
    const out = wellknown(
      req('/.well-known/oauth-protected-resource/mushi-mushi/hosted-mcp', '', 'GET'),
    );
    assert.equal(out.statusCode, 200);
