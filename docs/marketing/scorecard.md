...
| Growth loop: "Bug reports by Mushi" widget mark (link + `loop_impression`/`loop_click`), runtime `widget.brandFooter` (Free Cloud on, paid/self-host off), console tri-state toggle, RN + Node `track()` | **repository green**, server side deployed | migration `20260921000006`; web bundle 88.96 kB under the committed 89.5 KB budget (`packages/web/package.json` size-limit) |
