...
Migration `20260806035756_harden_byok_validation_lifecycle.sql` enforces
validate-before-activate. Before applying it to an existing self-host, inventory
active keys that have no successful probe:

```sql
select project_id, provider_slug, count(*) as keys_requiring_retest
from public.byok_keys
where status = 'active' and test_status is distinct from 'ok'
group by project_id, provider_slug;
```

The migration intentionally moves those rows to `pending_validation`; it never
deletes their Vault secrets. After deploy, operators must use Settings → API
Keys → **Test** (or `mushi keys test <key-id>`) to probe and reactivate each
credential. Runtime resolution excludes pending keys so unverified credentials
cannot silently reach a provider. Record a zero-row preflight or the completed
retest list in the deployment change log.
