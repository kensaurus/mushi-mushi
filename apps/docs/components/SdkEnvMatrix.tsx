  ...
  return (
    <div className="not-prose overflow-x-auto">
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="border-b border-[var(--docs-rule)]">
            <th className="text-left py-2 pr-4 font-semibold">Surface</th>
            <th className="text-left py-2 pr-4 font-semibold">Variables</th>
            <th className="text-left py-2 font-semibold">Notes</th>
          </tr>
        </thead>
        <tbody>
          {ROWS.map((r) => (
            <tr key={r.framework} className="border-b border-[var(--docs-rule)]/60">
              <td className="py-2 pr-4 align-top whitespace-nowrap">{r.framework}</td>
              <td className="py-2 pr-4 align-top">
                <code className="text-xs">{r.vars}</code>
              </td>
              <td className="py-2 align-top text-[var(--docs-ink-muted)]">{r.notes ?? '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
