...
**Vault encryption:** BYOK key values are stored as `vault://<id>` references pointing to Supabase Vault (`vault.secrets`). The `project_settings.byok_*_key_ref` columns never hold raw key strings in production (enforced by `dereferenceKey()` in `_shared/byok.ts` which rejects non-vault refs in the `production` environment).
...
**Control (Wave 5 Gap F):** The `vault_store_secret(name, value, project_id)` Postgres function now enforces the naming convention `mushi_<project_id>_*` when `project_id` is supplied. A misconfigured caller cannot overwrite a different tenant's secret. The three-arg form is used by all admin routes that manage BYOK keys.
