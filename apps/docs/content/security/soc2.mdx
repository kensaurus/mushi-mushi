...
```sql
-- Via the SQL editor or MCP
select soc2_evidence_snapshot();
```

Or click **Regenerate** in the admin console. The new pack appears in `soc2_evidence` within ~30 seconds.
...
```sql
select request_dsar('user@example.com');
-- Returns a signed URL to a tarball of all rows referencing that email.
```

The tarball includes: `reports`, `reporter_identities`, `activity_events`, `audit_log` rows where `actor_email = ?`. PII columns are decrypted in the export (the tarball itself is signed and expires in 48h).
