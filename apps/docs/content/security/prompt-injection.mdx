...
<Callout type="info">
  The regression suite in `packages/server/tests/injection.test.ts` runs the full OWASP LLM01 corpus on every CI push. A new injection pattern entering `sanitizeForLLM()` is a test failure, not a PR merge.
</Callout>
...
`packages/server/tests/injection.test.ts` covers:
...
```bash
cd packages/server
npm test -- --testPathPattern injection
```
