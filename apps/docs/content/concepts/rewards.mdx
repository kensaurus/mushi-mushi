...
The Rewards system transforms the Mushi SDK from a passive collection tool
into an engagement loop. Users earn points for activity, climb tiers, and
unlock perks — while you collect richer, higher-quality reports from users who
...
- `navigate` — explicit `<a>` clicks and `router.push()` calls
