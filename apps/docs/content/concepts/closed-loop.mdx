...
This page is the long-form version of the argument the landing page makes in one paragraph. Read it if you want to understand *why* Mushi is designed the way it is, not just *what* it does.
...
1. **A user feels friction** (a dead button, a slow screen, a layout that breaks on one phone) and shakes the app or clicks the feedback stamp.
2. **Mushi SDK captures** the moment: screenshot, session breadcrumbs, device info, the current route.
3. **The server embeds the report** with `text-embedding-3-small` and runs the BIRCH-style streaming clusterer.
4. **Similar reports collapse** into a `mistake_cluster`. When the cluster reaches coherence ≥ 0.75 and size ≥ 3, the judge model promotes it to a **`lesson`** with auto-generated name + summary + one-shot rule.
5. **The lesson is injected** into the next PR review (token-budget packed to ≤ 3 000 tokens) and into the next agentic fix run — so neither human reviewers nor the AI agent can repeat the same class of mistake.
6. **The user is credited** in the changelog: *"Fixed by Kenji: the Settings page back-button was double-counting the safe-area inset"*. The SDK shows a toast on the next session.
7. **Drift and anomaly detectors** run continuously, generating candidate reports for issues no user has noticed yet. Those reports enter the same pipeline at step 2 — the loop closes without waiting for a human to file a ticket.
