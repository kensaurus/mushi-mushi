...
| --------------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Cursor Cloud Agents**                             | Marketplace plugin or `cursor_cloud` | Two paths: install _Cursor Cloud Agent_ from the Marketplace to dispatch agents on `report.classified`/`qa_story.failed`; or set `autofix_agent=cursor_cloud` in project settings to make Cursor the default fixer. See [Admin → Integrations → Cursor Cloud](/admin/integrations). |
| **Cursor IDE / stdio MCP**                         | stdio MCP                            | ~20 tools instantly. Add `@mushi-mushi/mcp` to `mcpServers` in `.cursor/mcp.json`. Use `dispatch_fix --agent cursor_cloud` from the Cursor chat to trigger a Cursor Cloud Agent from inside the IDE.               |
