...
| Docs | `mushi_events_consent_<projectId>` | Remember your answer to the analytics consent bar | Until cleared |
| Docs | Release-banner dismissal, theme preference | Hide a banner you closed; light / dark | Until cleared |
| **Your app** (SDK) | `mushi:reporter-token:<projectId>` | An opaque token so repeat reports from the same browser group together. It is not an email or a name | Until cleared |
