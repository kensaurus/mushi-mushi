...
| Docs | `mushi_events_consent_<projectId>` | Remember your answer to the analytics consent bar — the only analytics key written before you answer | Until cleared |
...
| **SDK** (your app, the console, the docs after consent) | `mushi:reporter-token:<projectId>` | A random token so repeat reports and analytics events from the same browser group together. It is not an email or a name, but it links events from the same browser | Until cleared |
