...
The **Cursor Cloud Agent** plugin (`cursor-cloud-agent`) listens to `report.classified`, `fix.requested`, and `qa_story.failed`. When these fire, it dispatches a Cursor Cloud Agent run instead of a standard outbound webhook — no `webhookUrl` is required.
