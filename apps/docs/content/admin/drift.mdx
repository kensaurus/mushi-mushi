...
3. Is this a regression? → Click **→ Lesson** to create a candidate, then go to
   [Fix orchestrator](/admin/fixes) → dispatch a fix directly.
...
- [Iterate (PDCA)](/admin/iterate) — run a full quality scoring loop after clearing drift findings
- [Inventory & user stories](/admin/inventory) — update the baseline snapshot
- [Fix orchestrator](/admin/fixes) — act on critical findings
