...
The server (`@mushi-mushi/server`, `@mushi-mushi/agents`, `@mushi-mushi/verify`) is now **AGPLv3**.
...
We can now say "the server is open source" and mean it. The SDKs were always MIT. The distinction is: SDK = embed anywhere; server = copyleft when you SaaS it.

That's Mushi's licensing model going forward. No change dates, no "convert to Apache in X years," no ambiguity.
