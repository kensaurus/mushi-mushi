...
import { QUICKSTART_ONE_KEY_CALLOUT } from '@/lib/public-copy'
...
`<MushiProvider>` uses React context, so it goes in a client component, not
...
soon as `<MushiProvider>` initialises.
