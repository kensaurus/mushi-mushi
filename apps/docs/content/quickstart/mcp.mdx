...
This reads `~/.mushirc` (written by `mushi login`), detects the IDE config
path, and writes the `mcpServers.mushi` block for you. Restart the IDE and
ask the agent: **"list mushi tools"**.
...
<Callout type="tip">
  Get the pre-filled snippet in one click: Admin console →
  **Settings → API Keys** → **Copy as `.cursor/mcp.json`**.
</Callout>
