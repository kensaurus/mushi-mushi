...
All SDKs are at `0.8.x`. The JS packages are MIT-licensed; the server packages (`@mushi-mushi/server`, `@mushi-mushi/agents`, `@mushi-mushi/admin`) are BSL — free for self-hosting, commercial license required for hosted-as-a-service redistribution.
