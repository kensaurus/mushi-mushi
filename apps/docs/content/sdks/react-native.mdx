...
The SDK captures **before** the bottom sheet overlays the screen. Users can
remove the thumbnail before submit. On finance/health apps, keep
`capture.screenshot: false` in config for screens that must never be photographed.
