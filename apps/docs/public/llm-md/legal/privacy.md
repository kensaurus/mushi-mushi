...
- **(f) legitimate interests** — keeping the service secure, preventing abuse
  ...
  expire (section 7).
- **(a) consent** — docs-site analytics. Before you answer the consent bar
  the docs site loads no analytics code, sends nothing, and stores nothing
  except your answer; Do Not Track and Global Privacy Control are honoured as
  a refusal. "No thanks" keeps it that way.
...
| Docs | `mushi_events_consent_` | Remember your answer to the analytics consent bar — the only analytics key written before you answer | Until cleared |
...
| **SDK** (your app, the console, the docs after consent) | `mushi:reporter-token:` | A random token so repeat reports and analytics events from the same browser group together. It is not an email or a name, but it links events from the same browser | Until cleared |
...
| **SDK** | Offline report queue (IndexedDB `mushi-mushi`, falling back to `mushi_offline_queue`) | Reports that could not be sent yet | Until sent |
