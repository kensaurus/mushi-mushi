...
- **`<HotkeysModal>`** (`src/components/HotkeysModal.tsx`) — `?`-triggered cheatsheet of every keyboard shortcut in the admin, grouped by scope (Global / Navigate / Reports / Fixes). Single source of truth for the shortcut registry — adding a hotkey is one entry in this file, not a grep across forgotten help overlays. Renders through the shared `Modal` so focus-trap / backdrop / Esc behave identically to every other dialog
