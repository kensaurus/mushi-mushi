...
Foreground / border tokens are calibrated against the canonical surface in each theme so dividers, helper text, and section labels clear WCAG 1.4.11 (≥3:1 non-text) and 1.4.3 (≥4.5:1 normal text). After the 2026-05-07 contrast pass the dark-mode minimums are: `fg-faint` ~4.4:1, `fg-muted` ~5.0:1, `edge` ~3.3:1, `edge-subtle` ~3.0:1, `nav-section-label` ~4.5:1 (the four PDCA group headers in the sidebar). Light-mode dividers (`edge` 0.78 / `edge-subtle` 0.84) are now visible at a glance instead of fading into the page surface. A global `:focus-visible { outline: 2px solid var(--color-brand); outline-offset: 2px }` rule in `src/index.css` is the safety net for any element that didn't opt into a Tailwind `ring-*` utility — it satisfies WCAG 2.4.13 (Focus Appearance, 2 px solid + ≥3:1) and stacks under explicit `outline-none` rules so existing component rings render unchanged.
...
- **`CopyButton`** — icon-only one-click clipboard primitive (16×16 chrome, 11×11 glyph; `size="md"` swaps to 28×28). Ships in two modes: `value` mode (we own the clipboard write + the 1.5 s green-check pulse) and controlled `onCopy + copied` mode for callers that need to pair the button with their own toast / banner / parent-managed feedback. Forwards `data-testid` so legacy Playwright assertions (`mcp-snippet-copy`) keep working. **Single visual language** — every "Copy / Copied!" *text* button across the admin (`SdkInstallCard`, `OnboardingPage` key reveal, `SetupGatePage` env blocks, `McpPage` snippets, `CodebaseIndexCard` webhook secret, `CopyViewLinkButton`) renders through this primitive instead of bespoke per-page chrome
- `InfoHint` — `i`-icon tooltip for inline use
...
`/organization/members` is the self-serve roster for Pro and Enterprise organizations. Hobby and Starter render the new layered **upgrade-nudge** UX (see [Upgrade nudges](#upgrade-nudges-feature-gating-without-hiding) below) and the backend returns `402 feature_not_in_plan` for invite attempts. The plan check now reads from `useEntitlements().has('teams')` instead of a hardcoded `PAID_TEAM_PLANS` set, so legacy grandfathered plans grant teams without a code change. The page reads `/v1/org/:id/members`, creates invites through `/v1/org/:id/invitations`, and accepts invite links at `/invite/accept?token=...`.

Locked-state composition (Hobby / Starter):

- A single editorial **`<UpgradeBanner flag="teams" density="comfy">`** above the form spells out what Pro unlocks and the actual `$X/mo` price.
- The invite form stays mounted but is wrapped in **`<UpgradeLockOverlay flag="teams">`** — children render dimmed + un-pointer-eventable with a centred lock + Upgrade CTA, so the user *sees* the form they'd interact with after upgrading instead of hitting "broken-looking" disabled inputs.
- Non-admin members on a Pro plan see a tooltip-explained disabled `Invite` button ("Only owners and admins can invite teammates") instead of a silent no-op.
...
### Upgrade nudges (feature gating without hiding)

Three reusable primitives in `src/components/billing/UpgradeNudge.tsx` translate `useEntitlements().has(flag)` checks into in-context upsells without hiding the gated surface from the user. Replaces the previous "filter the row out of the sidebar" approach where Hobby plans literally couldn't see what Pro unlocks. All three self-noop when the user already has the feature, so callers don't have to gate-check the call. CTAs deep-link to `/billing?focus=<flag>` so the billing page can scroll the matching plan into view.

- **`<UpgradeBanner flag>`** — single-row strip with key icon + tagline + `Upgrade to <plan> — $X/mo` CTA. Use above gated forms (e.g. invite-teammate, install-plugin). `density="comfy"` adds a "You're on <plan>" sub-line.
- **`<UpgradePill flag>`** — tiny `PRO` / `ENT` badge. `as="auto"` (default) renders as a `<span>` with native `title` so the pill is safe to drop *inside* a parent `<Link>` (sidebar nav rows, table cells that link to a detail page); `as="link"` renders as a billing deep-link for standalone use.
- **`<UpgradeLockOverlay flag>`** — wraps an existing form/control, dims it, blocks pointer events, and centres an upgrade CTA over the children. Use when the gated surface is rich (form, chart, config panel) and you want to *show* the user what they'd get — same shape it would have unlocked — without letting them interact with it.

The full editorial-card variant **`<UpgradePrompt>`** (same file family, pre-existing) replaces a feature's whole form on `/sso`, `/byok`, `/plugins`, and `/intelligence`. The 402 fallback host **`<UpgradePromptHost>`** (mounted in `App.tsx`) listens for `mushi:entitlement-blocked` events from `apiFetch` and toasts a "View plans" link if a stale UI somehow reaches a gated mutation.

**Sidebar adoption** — `Layout.tsx` now annotates gated nav items with `requiresFeature` (`Members → 'teams'`, `SSO → 'sso'`, `Audit Log → 'audit_log'`, `Compliance → 'soc2'`) and renders an inline `<UpgradePill flag>` next to the label in advanced mode instead of filtering the row out. Quickstart / Beginner modes still trim aggressively — Pro pills in a 3-page Quickstart shell would be more confusing than helpful. The page itself still ships its own `<UpgradePrompt>` for unentitled users, so this is purely a *visibility-without-unlocking* change.
...
5. **Dashboard getting started** — when the user has 0 reports, `GettingStartedEmpty` renders a PDCA-framed first-run script (Plan → Do → Check) on top of the existing setup checklist, so the next action is always one click away. The `Install the SDK` row of the checklist now renders an **SDK heartbeat diagnostic strip** (driven by `project_api_keys.last_seen_*`, recorded by `apiKeyAuth` on every authed SDK call): green when the SDK has reached this admin's backend (`SDK was last seen 2 min ago from https://your-app.com (Chrome 138)`), warn-toned `BACKEND MISMATCH` when the SDK is reaching a *different* host than this admin reads from, and an info-toned "API key has never been used" nudge when the heartbeat is empty. This decouples "SDK installed" from "first user-triggered report" and turns the most common cross-environment misconfigurations into self-explaining UI
...
- **`src/lib/focusMode.ts > useFocusMode()`** + the focus-mode toggle in the sidebar footer of `Layout.tsx` — `Cmd/Ctrl+.` toggles the mode (or click the toggle in the sidebar footer); collapses the sidebar + sub-header + ribbons to give the active list / detail view full width. Persisted in `localStorage:'mushi:focus-mode:v1'`. Because the in-sidebar toggle is itself hidden once focus mode engages, a floating top-right `● Focus mode · Exit [Esc]` pill renders outside the sidebar as the always-visible escape hatch, and `Esc` exits focus mode globally (only fires while focus mode is on, so modals / inputs keep `Esc`)
- **`src/lib/sidebarCollapsed.ts > useSidebarCollapsed()`** + the collapse toggle pinned to the bottom of the desktop sidebar — `[` toggles between the full nav rail (208 px) and an icon-only rail (48 px). Persisted in `localStorage:'mushi:sidebarCollapsed:v1'`. The collapsed rail keeps every nav item and the active-route highlight; section headers compress to a thin divider per PDCA group, item labels become `title` tooltips with mirrored `aria-label` for SR users, and segmented controls that can't fit (density, theme) hide entirely. Mobile is unaffected — it always renders the full sidebar in an overlay. Mirrors Linear's collapsed-sidebar pattern, including the `[` hotkey for muscle-memory transfer
...
| `/onboarding` | First-run setup wizard (project, API key, test, SDK snippet). The active step is highlighted with a "do this next" chip + brand ring on the checklist row, and the banner version auto-collapses once required steps are done **or** completion ≥ 80%. The `Install the SDK` row also renders an **SDK heartbeat diagnostic strip** (`SDK HEARTBEAT` / `BACKEND MISMATCH` / never-seen) sourced from `project_api_keys.last_seen_*` so users can verify the SDK is reaching the right backend without sending a real bug report |
| `/reports` | Filterable report list (status / category / severity / `component` / `reporter`); top of page shows a **`ReportsKpiStrip`** with 14-day severity rollups; rows render a **`StatusStepper`** (`new → classified → fixing → fixed`) instead of a static badge, a 4 px left-edge severity stripe, a `+N similar` badge for deduped reports (driven by `report_group_id`), an **`unique_users` blast-radius column** powered by a `COUNT(DISTINCT)` Postgres RPC, a **"Reporter replied"** badge whenever `last_reporter_reply_at > last_admin_reply_at` so triage can spot threads that need a follow-up, and a single primary action button — `Triage →` / `Dispatch fix →` (gated on `DISPATCH_ELIGIBLE_STATUSES`). Group-by-fingerprint collapse is on by default (`?group=fingerprint`); expanded groups persist in `?expand=<id>` so deep links restore state. Each row's summary opens a **`BreadcrumbPeek`** (`src/components/reports/BreadcrumbPeek.tsx`) hover popover surfacing the last 5 SDK breadcrumbs + sticky tags + Sentry release/environment so triage can glance "what was happening just before this report" without opening it. Inline **`ObservabilityStrip`** beneath the component line shows the most informative tag and a Sentry trace-id pill at-a-glance. Server filters added: `?tag=k:v`, `?trace=…`, `?release=…`, `?sentryEnv=…` are all backed by the new GIN/btree indexes on `reports.tags` / `sentry_trace_id` / `sentry_release` |
| `/reports/:id` | Report detail — **`ScreenshotHero`** at the top (large zoomable screenshot), then a **`PdcaReceiptStrip`** that compresses the lifecycle into 4 stamps (Plan / Do / Check / Act) using `llm_invocations`, `fix_attempts`, and `classification_evaluations` data fetched in a single API round-trip. When a fix attempt exists, **`ReportBranchGraph`** (`src/components/report-detail/ReportBranchGraph.tsx`) renders right below the strip: a collapsible section that fetches `/v1/admin/fixes/:id/timeline`, reuses the existing `FixGitGraph` to draw the dispatch → branch → commit → PR → CI → merge timeline, and surfaces branch name (in `CodeValue`, hash tone), base branch, PR link, CI badge, files changed, commit SHA, and the Langfuse trace link. Open/closed state persists in `localStorage`; the block polls while the fix is still live (`queued` / `running`). Then: recommended next action, triage bar, LLM classification, environment, **`TimelineCard`** (chronological repro trail of route / click / screen events from `reports.repro_timeline`, with an empty-state nudge to upgrade the SDK when no trail was captured), console / network / performance (always rendered with empty states), **`SentryContextPanel`** (`src/components/report-detail/SentryContextPanel.tsx`) when the SDK shipped Sentry context: dual breadcrumb timeline interleaving Mushi + Sentry crumbs chronologically, copy-to-clipboard `event_id` / `replay_id` / `trace_id`, sticky tag chips, "Open in Sentry" deep-link, and a one-line explainer for the bidirectional `mushi.report_id` linking. The same panel renders inside `ReportPreviewDrawer` so triage gets the full Sentry context without a route change. Comment thread (admin + reporter messages tagged with `author_kind`, distinct visual treatment for reporter replies, structured **`feedback_signal` badges** — `confirms` / `wrong_target` / `noise` — that turn unstructured "still broken" replies into machine-readable signals the prompt-tune loop consumes), related cross-links (component, reporter, graph, fix) |
...
| `/graph` | Knowledge graph — auto-switches between two views: a Sankey-style **`GraphStoryboard`** (left-to-right columns by `node_type` with bezier links + the **most-affected node** named under each column header + an inline edge-weight legend) when fewer than 12 nodes exist, and the full React Flow canvas otherwise. Filter chips are grouped (`Show node types` / `Connect via edges`) with `all` toggles. Minimap is suppressed on small graphs to avoid clutter. Toggleable "Table" view renders nodes + edges as accessible HTML tables for screen readers; canvas has `role="region"` + descriptive `aria-label`. A "Force canvas view" override is available when the storyboard threshold trips by accident. Adds a Mushi v2 **Surface** mode that overlays the positive-side inventory (apps → user-stories → pages → elements → actions) on top of the same canvas so triage can see "this report broke action X under user-story Y" without leaving the page |
| `/inventory` | **Bidirectional inventory & gates** (Mushi v2 / v2.1, `src/pages/InventoryPage.tsx`). Tabbed surface for the positive-side inventory: **Discovery** (default), **Inventory tree**, **Drift**, **Gates**, **Synthetic**, **Settings**. The **Discovery tab** (`components/inventory/DiscoveryTab.tsx`) is a four-step `DiscoveryLifecycle` stepper — `Install → Observe → Propose → Accept` — with semantic done / active / pending states; below it sits a `DiscoveryMetrics` row (events, routes, users, freshness as semantic hero tiles) and an `ActionCard` that calls `POST /v1/admin/inventory/:id/propose` once `ready_to_propose` flips, then opens the `ProposalReviewModal` (Stories / Why these stories / YAML tabs with editable raw YAML for drafts). Observed routes render as expandable `ObservedRouteCard`s with an info-tinted event-share bar, testid + API-path chip stacks. The **Inventory tree** tab renders the accepted YAML as a deeply nested `InventoryTree`; **Drift** shows the `DriftDiffPanel` (added / removed / changed actions vs the latest crawl + SDK observations) — backed by the new hourly `mushi-inventory-drift-watch` cron that auto-invokes `inventory-propose` in `drift_watch` mode whenever the discovered inventory diverges from the accepted YAML, so a stale `inventory.yaml` no longer requires a manual nudge; **Gates** lists `GateFindingCard`s for every `gate_findings` row; **Synthetic** plots `SyntheticTimeline` for each declared user-story; **Settings** wires `crawler_base_url`, `crawler_auth_config`, and `synthetic_monitor_enabled` (the auth-config blob is the one [`@mushi-mushi/inventory-auth-runner`](../../packages/inventory-auth-runner) writes back to). Empty state offers two paths — **Open Discovery →** for SDK-driven flow and **Paste YAML** (`InventoryYamlDropzone`) for direct ingest |
...
| `/fixes` | Auto-fix PDCA — KPI summary (last 30d), daily volume sparkline, per-fix branch graph (`FixGitGraph`) overlaying dispatch → branch → commit → PR → CI → merge, retry button. The KPI row now carries a **"Why fixes failed"** tile that surfaces the dominant `failure_category` (e.g. `sandbox_timeout`, `llm_invalid_json`, `github_403`, `scope_blocked`) with a hover-expanded histogram of every other category, so operators can prioritise prompt / scope tuning without grepping logs. Each failed `FixCard` also renders the categorised reason as a badge alongside the raw error message |
| `/projects` | Project management + API keys, with toast feedback for create / generate / revoke. Each project card surfaces a **`PdcaBottleneckPill`** (Plan / Do / Check / Act tone) labelled with the most-urgent stalled stage and deep-linking straight to that page (e.g. "3 fixes need retry → /fixes"). A **`ProjectContextStrip`** sits in the metadata row exposing the rich project surface added in 1.0: linked Git repo (URL, branch, indexing status, error count from `project_repos`), codebase file count, **30-day severity breakdown** (`Critical / High / Medium / Low`), **7-day report-volume trend arrow** (`↑`/`↓` with delta), plan-tier + data-residency badges, and a **Sentry-connected** chip whenever ≥1 report in the last 30d shipped a `sentry_trace_id`. The metadata row also carries an **`SdkVersionBadge`** (`src/components/SdkVersionBadge.tsx`) — a small `web v0.7.0 → v0.9.0` chip with three tones (`up-to-date` / `outdated` / `deprecated`) sourced from the project's most recent `reports.sdk_version` joined against the `sdk_versions` catalogue. Renders nothing for `unknown` (no reports yet) so quiet projects don't blink red before the first ingest; full version + deprecation message live in the hover tooltip. Newly minted API keys render through **`RevealedKeyCard`** (`src/components/RevealedKeyCard.tsx`) — the plain-text secret is shown **once**, with tabbed "copy as" output for the three real consumption modes: raw token, a `MUSHI_API_KEY=…` block for `.env.local` / CI, and a full `.cursor/mcp.json` snippet pre-filled with the project id so the user can paste it into a repo and the agent Just Works. Scope badges are inlined so `mcp:read` vs `mcp:write` is obvious before dismissal, and a "Learn more" link points at `/mcp` for the full catalog. **`SdkInstallCard`** (`src/components/SdkInstallCard.tsx`) ships an expandable **Power-user APIs** disclosure block with copy-paste examples for `Mushi.identify()`, `Mushi.setTag()` / `setTags()`, `Mushi.addBreadcrumb()`, and `Mushi.captureException()` so the install snippet doubles as the discovery surface for the new observability APIs |
...
| `/docs-bridge` | **Auth bridge for the docs Migration Hub** (`src/pages/DocsBridgePage.tsx`) — popup target opened by `apps/docs/lib/migrationProgress.ts > openAdminAuthBridge()`. Mounted INSIDE `ProtectedRoute`, so an unauthenticated visitor lands on `/login?next=/docs-bridge?nonce=...` first and only ever reaches the bridge with a real session. The page validates `nonce`, normalises `returnOrigin` against an allowlist (kensaur.us / localhost), and `postMessage`s `{ type: 'mushi:docs-bridge:token', nonce, accessToken, apiUrl, projectId, organizationId, expiresAt, email }` back to the docs opener — **never** a refresh token. Auto-closes the popup after a successful handoff and renders friendly status copy on missing-opener / opener-origin-mismatch / nonce-missing failures so a paste of the bridge URL into a fresh tab degrades gracefully |
...
- **`PipelineStatusRibbon`** (`src/components/PipelineStatusRibbon.tsx`) — Advanced-mode-only "pipeline pulse" strip mounted once at the top of every authenticated page (suppressed in Quickstart / Beginner where `<NextBestAction>` covers the same job). Four tone-coloured tiles mirror the PDCA stages with one metric + one-line summary each, deep-linking to the page that owns the stage. Backed by `useNavCounts` so the ribbon shares the realtime subscription with the sidebar health dots. **Collapsible**: a `Collapse ▴` toggle persists the choice to `localStorage:'mushi:pipelineRibbon:collapsed:v1'`; collapsed state renders a single-line pill that surfaces the *worst-tone* stage so a `danger` Plan still grabs the eye after collapsing — operators reclaim ~50 px of vertical real estate per page without losing the "is anything red?" signal. Beginner mode never sees the ribbon, so the persistence is per-power-user, not global
...
- **`useDensity()`** (`src/lib/useDensity.ts`) — three-way UI density (`compact` / `default` / `comfortable`). Persisted in `localStorage:'mushi:density:v1'`, applied as `data-density` on `<html>`. Works alongside **`TableDensityToggle`** in `ResponsiveTable.tsx`, which reuses the same micro-segmented track in inline mode.
...
- `GET  /v1/admin/projects` — each project row carries `pdca_bottleneck` + `pdca_bottleneck_label`, computed from `reports` (Plan), `fix_attempts` (Do), and `classification_evaluations` (Check). Powers the `PdcaBottleneckPill`. Also returns SDK freshness columns — `sdk_package`, `sdk_version` (the most recent report's `reports.sdk_package` / `reports.sdk_version`), `sdk_latest_version` + `sdk_deprecation_message` (joined from the `sdk_versions` catalogue), and a derived `sdk_status: 'up-to-date' \| 'outdated' \| 'deprecated' \| 'unknown'` so the FE renders a single chip without re-running the comparison. The response envelope also ships a top-level `latest_sdk_versions: Record<package, { version, deprecated, deprecation_message, released_at }>` so install-snippet feedback can read the catalog even before any report has landed. Powers `SdkVersionBadge`
...
When changing admin console UI (`apps/admin/src`), verify:
