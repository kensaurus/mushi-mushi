...
### Workspace preferences (theme, density, what's new, AI sidebar)
...
- **Page-context registry** (`src/lib/pageContext.ts`) + **`usePublishPageContext()`** / **`usePageContext()`** — shared pub/sub for "what is the user looking at *right now*?" Pages publish `{ route, title, summary, filters, selection, actions, questions }`; the AI sidebar, hotkeys modal, and command palette all subscribe to the same snapshot so their behaviour tracks the active page instead of being statically keyed off the URL. Uses a module-level store + `useSyncExternalStore` (StrictMode-safe, zero provider wiring) — pages that don't care simply never publish, and consumers fall back to a route-based default. `filters` are sparse-rendered as monospace chips in the AI sidebar header so the user can confirm the assistant sees what they see before asking. Currently published by **Reports** (status / severity / category / search / group + focused or previewed report + triage-next / clear-filters / select-all actions) and **Fixes** (current status bucket + in-flight / failed counts + expanded fix id)
- **`<AIAssistSidebar>`** (`src/components/AIAssistSidebar.tsx`) — `Cmd/Ctrl+J`-triggered chat panel scoped to the current page. Reads from the page-context registry and sends `{ route, context: { title, summary, filters, selection }, messages }` to `POST /v1/admin/assist` — so the backend prompt can condition on real state like "user is on /reports, filtered to status=new severity=critical, looking at report `55d36e23`" instead of guessing from the URL. The drawer header shows the live page title, a context strip renders the summary + filter chips + focused entity, empty-state suggestions are drawn from `ctx.questions` (the page tailors them to its filters — e.g. "Why did the 3 failed fixes fail?" when `failed > 0`) and fall back to a static route-based list otherwise, and page-contributed `actions` render as one-tap quick-action chips below the suggestions. When the endpoint isn't deployed the sidebar detects a `not_found` / `404` error code and degrades gracefully to "AI assist isn't available on this workspace yet" — safe to ship ahead of the backend
