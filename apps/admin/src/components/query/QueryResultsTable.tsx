...
function renderCell(col: string, val: unknown): ReactNode {
  ...
  if (colL.endsWith('_url') && typeof val === 'string' && val.startsWith('http')) {
    ...
    return (
      <a href={val} target="_blank" rel="noopener noreferrer" className="text-accent-foreground hover:text-accent underline underline-offset-2 motion-safe:transition-colors underline-offset-2 hover:text-brand/80 transition-colors" title={val}>
        {short}{val.length > 36 ? '…' : ''}
      </a>
    )
  }
