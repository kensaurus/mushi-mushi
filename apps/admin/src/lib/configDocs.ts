...
const SETTINGS_GENERAL: ConfigDoc[] = [
  ...
  },
  {
    ...
    summary:
      'When enabled, Sentry user-feedback submissions are mirrored into the Mushi report queue.',
    howItWorks:
...
const SETTINGS_DEVTOOLS: ConfigDoc[] = [
  {
    ...
    label: 'Debug Mode',
    summary:
      'Local-only toggle that prints every API call, auth event, and timing to the browser console.',
    howItWorks:
...
const PROJECTS: ConfigDoc[] = [
  ...
  },
  {
    ...
    summary:
      'Lets the MCP server expose read-only resources (reports, projects, prompts) to LLM clients.',
    howItWorks:
...
const INTEGRATIONS: ConfigDoc[] = [
  {
    ...
    summary:
      'Identifies your Sentry organization in API calls — the bit after `sentry.io/organizations/`.',
    howItWorks:
  ...
  },
  {
    ...
    summary:
      'The specific Sentry project Mushi correlates against for stack traces and breadcrumbs.',
    howItWorks:
  ...
  },
  {
    ...
    summary:
      'GitHub App installation token (preferred) or fine-grained PAT used to push branches and open PRs.',
    howItWorks:
    ...
    whenToChange:
      'Re-issue when the GitHub App is uninstalled/reinstalled, or when a PAT hits its expiry.',
  },
  {
  ...
  },
  {
    ...
    howItWorks:
      'Stored as a vault reference (`vault://id`). The health probe calls `GET api.cursor.com/v0/me`; fix-worker and the Node orchestrator use the same key to `POST /v0/agents` with your GitHub repo URL. No workspace ID is required by the official Cursor API.',
    default: { value: 'unset (Cursor dispatch disabled)' },
    backend: {
      table: 'project_settings',
      column: 'cursor_api_key_ref',
      endpoint: 'PUT /v1/admin/integrations/platform/cursor_cloud',
      readBy: ['integration-health-probe', 'fix-worker', 'Node orchestrator'],
    },
  ...
  },
  {
    ...
    howItWorks:
      'Passed as `model` in the v0 agents payload. Leave blank to use your Cursor account default (typically composer-2.5).',
    default: { value: 'composer-2.5 (account default when blank)' },
    backend: {
      table: 'project_settings',
      column: 'cursor_default_model',
      endpoint: 'PUT /v1/admin/integrations/platform/cursor_cloud',
      readBy: ['fix-worker', 'Node orchestrator'],
    },
  ...
  },
  {
    ...
    howItWorks:
      'Maps to `target.autoCreatePr` in the v0 agents API. Disable if you prefer to review the branch before opening a PR.',
    default: { value: 'true' },
    backend: {
      table: 'project_settings',
  ...
  },
  {
    ...
    howItWorks:
      'Higher values can recover from a first-pass miss but consume more Cursor API credit. Stored in project_settings; the v0 API may cap iterations server-side.',
  ...
  },
  {
    ...
    howItWorks:
      'Mushi never embeds this key in repository_dispatch payloads or public workflow YAML. After you save it here, copy the mushi-claude-fix workflow into your repo and add ANTHROPIC_API_KEY as a GitHub Actions secret.',
    default: { value: 'unset' },
    backend: {
      table: 'project_settings',
      column: 'claude_api_key_ref',
  ...
  },
  {
    ...
    howItWorks:
      'Passed as `model` in the dispatch payload. Your workflow may pass it to Claude Code CLI or ignore it if you pin a model in the workflow file.',
    default: { value: 'claude-opus-4-1' },
    backend: {
      table: 'project_settings',
  ...
  },
  {
    ...
    howItWorks:
      'Must match `on.repository_dispatch.types` in your workflow. Mushi sends this event type when dispatching a fix; a mismatch means GitHub accepts the POST but no workflow runs.',
  ...
  },
  {
    ...
    howItWorks:
      'Sent as `target_branch` in the dispatch payload. Override when your default branch is not `main`.',
    default: { value: 'main' },
    backend: {
      table: 'project_settings',
    ...
    whenToChange: "Change when your repo's default branch isn't `main`.",
  },
  {
  ...
  },
  {
    ...
    summary:
      'Atlassian account email that owns the API token below — Jira pairs them as basic auth.',
    howItWorks:
  ...
  },
  {
    ...
    summary:
      'Short uppercase code that prefixes every issue in the target project (e.g. `BUG`, `MUSHI`).',
    howItWorks:
  ...
  },
  {
    ...
    summary:
      'Org or user that owns the issue-tracker repo (the bit before the slash in `owner/repo`).',
  ...
  },
  {
    ...
    summary:
      '32-character integration key for PagerDuty Events API v2 — pages on-call when severity = critical.',
    howItWorks:
...
const STORAGE: ConfigDoc[] = [
  {
    ...
    summary:
      'Which object-storage backend hosts user-uploaded artifacts (screenshots, recordings, attachments).',
    howItWorks:
  ...
  },
  {
    ...
    label: 'Endpoint',
    summary:
      'Full URL of the storage API endpoint — only needed for non-AWS S3-compatible providers.',
    howItWorks:
  ...
  },
  {
    ...
    label: 'Path prefix',
    summary:
      'String prepended to every key the storage adapter writes — segments artifacts inside a shared bucket.',
    howItWorks:
  ...
  },
  {
    ...
    summary:
      'Lifetime of presigned download URLs handed out to admin users for screenshots and recordings.',
    howItWorks:
...
const PROMPT_LAB: ConfigDoc[] = [
  ...
  },
  {
    ...
    summary:
      'How many synthetic test cases to generate when seeding evals for a new prompt version.',
    howItWorks:
...
const ANTI_GAMING: ConfigDoc[] = [
  {
    ...
    summary:
      'Show only flagged reports, only clean ones, or both — scopes the table without losing your sort state.',
    howItWorks:
  ...
  },
  {
    ...
    summary:
      'Collapses runs of byte-identical reports into a single grouped row with an occurrence count.',
    howItWorks:
    ...
    default: { value: 'on' },
    whenToChange:
      "Turn off when you need to see the timing distribution of a flood (the per-row timestamps tell you about cadence, the grouped view doesn't).",
  },
  {
...
const NOTIFICATIONS: ConfigDoc[] = [
  {
    ...
    label: 'Show filter',
    summary:
      'Picks which notifications appear in the list — `all`, `unread only`, or per-category.',
    howItWorks:
  ...
  },
  {
    ...
    label: 'Type filter',
    summary:
      'Filter the list to a single notification type (severity escalation, autofix opened, plugin error, etc.).',
    howItWorks:
      "Reads `?type=` from the URL. Multi-select isn't supported — pick one type at a time.",
    default: { value: 'all types' },
    whenToChange:
      'Use when chasing one class of notification (e.g. all "autofix opened") — keeps your inbox legible while you bulk-review.',
  },
];
...
const INTELLIGENCE: ConfigDoc[] = [
  {
    ...
    summary:
      'When ON, your project contributes anonymised aggregate metrics to cross-customer benchmarks (which then power the Intelligence page comparisons).',
    howItWorks:
...
const MCP: ConfigDoc[] = [
  {
    ...
    label: 'Snippet mode',
    summary:
      'Picks which install snippet you copy — `mcp.json` (Claude Desktop / Cursor) vs `.env.local` (custom MCP host).',
    howItWorks:
    ...
    whenToChange:
      "Use `mcp.json` for off-the-shelf clients. Switch to `.env.local` only if your MCP host doesn't parse `mcp.json` natively.",
  },
];
...
const SDK_INSTALL: ConfigDoc[] = [
  {
    ...
    summary:
      "Which corner of the user's app the bug-capture trigger pins to — top-left, top-right, bottom-left, bottom-right.",
    howItWorks:
    ...
    whenToChange:
      "Pick the corner that doesn't collide with your existing chrome (chat bubbles usually live bottom-right, so move Mushi to bottom-left if so).",
  },
  {
  ...
  },
  {
    ...
    label: 'Smart hide',
    summary:
      'Lets the launcher shrink, hide, or become an edge tab on mobile and while the user scrolls.',
    howItWorks:
  ...
  },
  {
    ...
    default: { value: 'auto' },
    whenToChange:
      'Force `light` or `dark` when your app explicitly ignores system preference (rare). Otherwise stay on auto for the best a11y default.',
  },
  {
    id: 'sdk-install.trigger_text',
    label: 'Trigger text',
    summary:
      'Short label shown next to the bug-capture trigger button (or hidden if the button is icon-only).',
    howItWorks:
    ...
    whenToChange:
      'Localise to your audience\'s language. Lengthen to "Report a bug" for novice users; shorten to "" for power users.',
  },
  {
    ...
    default: { value: 'on' },
    whenToChange:
      'Turn off in apps that log secrets to the console (rare and a smell). Keep on for everyday debugging.',
  },
  {
    ...
    whenToChange:
      'Disable on apps with PII-laden API traffic (medical, financial). Enable bodies temporarily when investigating a hard-to-repro API bug.',
  },
  {
    id: 'sdk-install.capture_performance',
    label: 'Capture performance',
    summary:
      'Attaches Web Vitals (LCP, INP, CLS) and recent `PerformanceObserver` entries to bug reports.',
    howItWorks:
    ...
    default: { value: 'on' },
    whenToChange:
      'Keep on for consumer apps where perf matters. Turn off for internal tools where noise outweighs signal.',
  },
  {
    id: 'sdk-install.capture_element_picker',
    label: 'Capture element picker',
    summary:
      'Lets the user click an on-page element while filing a report; the CSS selector is attached.',
    howItWorks:
    ...
    default: { value: 'on' },
    whenToChange:
      'Turn off in apps where users describe bugs textually (forms, dashboards). Keep on for visually-rich apps where pointing > typing.',
  },
  {
    id: 'sdk-install.screenshot_mode',
    label: 'Screenshot mode',
    summary:
      'When the SDK captures a screenshot — when the report opens, always (continuous), or never.',
    howItWorks:
...
export const CONFIG_DOC_GROUPS: ReadonlyArray<{
  route: string;
  label: string;
