...
export function backlogTooltip(stats: DashboardStats, opts: Opts = {}): MetricTooltipData {
...
export function reports14dTooltip(stats: DashboardStats, opts: Opts = {}): MetricTooltipData {
  ...
  return metricTip(
    plain
      ? 'How many bug reports arrived in the rolling last 14 days (UTC).'
    ...
    !stats.hasData && stats.setupDone
      ? {
          tone: 'info',
          text: plain
            ? 'Setup is complete but no recent reports — verify the SDK heartbeat and send a test report.'
            : 'Setup is complete but no recent intake — verify the SDK heartbeat and send a test report.',
        }
      : undefined,
  )
}

export function reports14dDetail(stats: DashboardStats, opts: Opts = {}): string {
  const plain = opts.plainLanguage ?? false
  return stats.hasData ? 'Intake active' : plain ? 'Waiting for reports' : 'Waiting for ingest'
}

export function fixesTooltip(stats: DashboardStats, opts: Opts = {}): MetricTooltipData {
  const plain = opts.plainLanguage ?? false
  const takeaway =
    stats.fixesFailed > 0
      ...
      : stats.fixesInProgress > 0
        ? plain
          ? `${stats.fixesInProgress} fix${stats.fixesInProgress === 1 ? '' : 'es'} queued or running.`
  ...
  if (stats.fixesFailed > 0) {
    ...
    callout = {
      tone: 'info',
      text: plain
        ? `${stats.openPrs} open PR${stats.openPrs === 1 ? '' : 's'} from completed fixes — merge or close when ready.`
        : `${stats.openPrs} open PR${stats.openPrs === 1 ? '' : 's'} from completed fixes — merge or close to advance Act.`,
    }
  }
...
export function focusTooltip(stats: DashboardStats, opts: Opts = {}): MetricTooltipData {
  const plain = opts.plainLanguage ?? false
  const stage = stats.focusStage
  const stageExplain: Record<string, string> = plain
    ? {
        plan: 'Plan — review backlog is the bottleneck.',
        do: 'Do — failed fix attempts need attention.',
        act: 'Act — one or more integrations are failing health checks.',
        check: 'Check — LLM calls are erroring in the 14-day window.',
      }
    : {
  ...
  const takeaway = stats.bottleneck
    ...
    : stats.setupDone
      ? plain
        ? 'All loop stages look balanced — no single bottleneck flagged in the last 14 days.'
        : 'All PDCA stages look balanced — no single bottleneck flagged in the last 14 days.'
      : `${stats.requiredComplete}/${stats.requiredTotal} onboarding steps done. Finish setup to unlock full loop metrics.`
  ...
  return metricTip(
    stats.focusLabel
      ? plain
        ? `Which loop stage needs attention right now — currently ${stats.focusLabel}.`
        : `Which PDCA stage needs attention right now — currently ${stats.focusLabel}.`
      : plain
        ? 'Which loop stage (Plan / Do / Check / Act) needs attention, or whether things are balanced.'
