...
export function SsoPage() {
  ...
  return (
    <div className={PAGE_CONTENT_STACK} data-testid="mushi-page-sso">
      <PageHeaderBar
      ...
      {ssoUnlocked && (
        <>
          <Card className="p-3 space-y-3">
            ...
            <Btn onClick={addProvider} disabled={submitting} loading={submitting}>
              Add Provider
            </Btn>
          ...
          {lastRegister?.status === 'registered' && (
            <Card className="p-3 border border-ok/30 bg-ok-muted/20 space-y-2">
              ...
              <p className="text-2xs text-fg-secondary">
                Paste these values into your identity provider so it can post SAML responses back to
                Supabase Auth.
              </p>
          ...
          {loading ? (
            ...
            <ErrorAlert message={`Failed to load SSO configs: ${error}`} onRetry={reload} />
          ) : configs.length === 0 ? (
            <EmptyState
