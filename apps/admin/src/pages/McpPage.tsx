    ...
    <div className="space-y-5">
      ...
      <Card className="p-5 space-y-4" data-testid="mcp-install">
        ...
        {/* ── Monorepo / project detector ───────────────────────────
            Paste package.json → auto-detect monorepo type and surface
            workspace-specific install guidance for the MCP binary.
            Completely optional; the snippet works without it. */}
        <div className="rounded-md border border-edge-subtle bg-surface-raised/40">
          ...
          >
            <span className="font-medium text-fg">
              🔍 Detect monorepo / workspace (optional)
            </span>
          ...
          {detectOpen && (
            <div className="px-3 pb-3 pt-1 space-y-2">
              <p className="text-2xs text-fg-muted leading-snug">
                Paste your repo's <code className="font-mono">package.json</code> to detect monorepo
                workspaces, and get the exact install command for your workspace.
              </p>
              <textarea
                ref={detectTaRef}
                value={detectText}
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => {
                    ...
                    const envInstall = `npm install -g mushi-mcp`
                    const note = monorepoInstallGuidance(result, envInstall)
                    setMonorepoNote(note)
          ...
          {!detectOpen && (monorepoNote || monoWarnings.length > 0) && (
            <div className="px-3 pb-3 space-y-1.5">
              ...
              <button
                type="button"
                onClick={() => { setMonorepoNote(null); setMonoWarnings([]) }}
